...
namespace crt
{
  ...
  void record_value::StaticMain(void *pParam) {
    ...
    msg.milliseconds = rtc.getMillis() + rtc.getSecond() * 1000 + rtc.getMinute() * 60000 + rtc.getHour() * 3600000;
      ...
      if (value != NULL){
        ...
        if (num_values % MQTT_MESSAGE_LENGTH == 0){
          ...
          msg.milliseconds = rtc.getMillis() + rtc.getSecond() * 1000 + rtc.getMinute() * 60000 + (rtc.getHour() - 1)* 3600000;
        }
      }
      vTaskDelay(1);
    }

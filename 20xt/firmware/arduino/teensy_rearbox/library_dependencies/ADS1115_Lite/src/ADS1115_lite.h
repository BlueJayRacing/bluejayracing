...
ADS1115 lite library - Adapted from adafruit ADS1015/ADS1115 library
        ...
        This library is stripped down version with bug fixes from the adafruit ADS1015/ADS1115 library in order to save
as much space as possible
                1. No explicit "Read ADC" functions exist to save space.  Simply set your mux manually, trigger a
conversion manually, and read the value manually.  See the example program
                2. ADS1015 functionality removed to save space.  Note that this library should work for the ADS1015, the
only difference is the sample rate constants.
                3. Continuous Conversion removed
                        a. If "continuous conversion" is required, perhaps you should rethink your requirements.
Continuous conversion is never REALLY required. b. That being said, if you want a continuous conversion simply start a
conversion after reading a value by executing "adc.triggerConversion()" immediatly AFTER an "adc.getConversion();".
That's all the chip does anyway...
                4. Comparator functionality removed to save space.  Cmon..you can program that functionality if you
REALLY need it, this is a lite library
                ...
                7. All report bug fixes from the adafruit library on github
                        a. Most notably the incorrect sample rate used when using the ADS1115.  Yes that's right, if you
used the Adafruit library the sample rate was set to the fastest, noisiest sample rate by default with no way to change
it!  Opps!
...
#define ADS1115_REG_CONFIG_DR_250SPS                                                                                   \
    (0x00A0) // 250 SPS, or every 4ms, note that noise free resolution is reduced to ~14.75-16bits, see table 2 in
             // datasheet
#define ADS1115_REG_CONFIG_DR_475SPS                                                                                   \
    (0x00C0) // 475 SPS, or every 2.1ms, note that noise free resolution is reduced to ~14.3-15.5bits, see table 2 in
             // datasheet
#define ADS1115_REG_CONFIG_DR_860SPS                                                                                   \
    (0x00E0) // 860 SPS, or every 1.16ms, note that noise free resolution is reduced to ~13.8-15bits, see table 2 in
             // datasheet
...
class ADS1115_lite {
  ...
  public:
    ...
    bool testConnection(); // returns 1 if ADS1115 is properly connected, 0 if not connected.  There is no point in
                           // calling any other function is this doesn't return a 1
    ...
    void triggerConversion(void); // Triggers a single conversion with currently configured settings in protected
                                  // variables.  Immediately returns
    bool isConversionDone(); // Polls ADS1115 for the conversion done register.  Returns 1 if conversion is finished, 0
                             // if in the middle of conversion.
    int16_t
    getConversion(); // Polls ADS1115 for the conversion done register.  Waits until it is finished and returns value.
};

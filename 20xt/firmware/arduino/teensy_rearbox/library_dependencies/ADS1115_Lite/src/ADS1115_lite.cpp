...
void ADS1115_lite::triggerConversion()
{
    ...
    config |= ADS1115_REG_CONFIG_MODE_CONTIN; // Single-shot mode (default)
...
{ // Wait for the conversion to complete
    ...
    return (
        (Wire.read() << 8) |
        Wire.read()); // Read each byte.  Shift the first byte read 8 bits to the left and OR it with the second byte.
}
/**************************************************************************/
/*!

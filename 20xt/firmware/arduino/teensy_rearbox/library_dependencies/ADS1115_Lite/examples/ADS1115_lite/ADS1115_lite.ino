...
ADS1115_lite adc(ADS1115_DEFAULT_ADDRESS); // Initializes wire library, sets private configuration variables to ADS1115
                                           // default(2.048V, 128SPS, Differential mode between  AIN0 and AIN1.  The
                                           // Address parameter is not required if you want default
...
void setup(void)
{
    ...
    // Typically the Gain and sample data rate should be set in setup as these typically don't change for a project, but
...
void loop(void)
{
    ...
    // The mux setting must be set every time each channel is read, there is NOT a separate function call for each
    // possible mux combination.
    ...
    adc.setSampleRate(
        ADS1115_REG_CONFIG_DR_8SPS); // Not truely required as this parameter was already configured in SETUP
    ...
    adc.setMux(
        ADS1115_REG_CONFIG_MUX_DIFF_0_1); // Set mux...this must be done each time you want to read a different channel

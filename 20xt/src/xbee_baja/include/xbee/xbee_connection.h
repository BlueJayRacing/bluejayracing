...
class XBeeConnection : public Connection {
  public:
    ...
    virtual int
    num_msgs_queued_for_tx() override; // Number of transmission requests sent to Xbee that haven't been ack'd by xbee
  ...
  private:
    ...
    static int tx_status_handler(xbee_dev_t* xbee, const void FAR* raw, uint16_t length, void FAR* conn_context);

    // Handle the dispatching of a received message. Adds the message to rx_queue
    static int receive_handler(xbee_dev_t* xbee, const void FAR* raw, uint16_t length, void FAR* conn_context);
};

...
XBeeConnection::XBeeConnection(const std::string serial_device, const int baudrate, const int cwnd_size)
    : serial_device(serial_device), baudrate(baudrate), CWND_SIZE(cwnd_size), num_queued_for_tx(0),
      last_acked_frame_id(0)
{
    ...
    this->xbee_frame_handlers =
        new xbee_dispatch_table_entry_t[]{{XBEE_FRAME_TRANSMIT_STATUS, 0, &XBeeConnection::tx_status_handler, this},
...
bool XBeeConnection::is_open() const { return this->conn_open; }
...
int XBeeConnection::tx_status_handler(xbee_dev_t* xbee, const void FAR* raw, uint16_t frame_length,
                                      void FAR* conn_context)
{
...
int XBeeConnection::receive_handler(xbee_dev_t* xbee, const void FAR* raw, uint16_t frame_length,
                                    void FAR* conn_context)
{
    XBeeConnection* this_conn         = (XBeeConnection*)conn_context;
...
xbee_serial_t XBeeConnection::init_serial(const std::string serial_device, const int baudrate)
{
    ...
    std::strncpy(serial.device, serial_device.c_str(), sizeof(serial.device));
    return serial;
}

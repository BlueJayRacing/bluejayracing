...
int xbee_ser_open(xbee_serial_t* serial, uint32_t baudrate)
{
    ...
    if (serial->fd <= 0) {
        ...
        if (serial->fd < 0) {
#ifdef XBEE_SERIAL_VERBOSE
            printf("%s: open('%s') failed (errno=%d)\n", __FUNCTION__, serial->device, errno);
#endif
            return -errno;
        }

...
#define ZDO_ENDPOINT(state) {WPAN_ENDPOINT_ZDO, WPAN_PROFILE_ZDO, zdo_handler, &state, 0x0000, 0x00, NULL}

// These functions are documented in zigbee_zdo.c.

wpan_ep_state_t FAR* zdo_endpoint_state(wpan_dev_t* dev);

int zdo_handler(const wpan_envelope_t FAR* envelope, // endpoint/profile/cluster/etc.
...
int zdo_match_desc_request(void* buffer, int16_t buflen, uint16_t addr_of_interest, uint16_t profile_id,
...
*/
int zdo_simple_desc_request(wpan_envelope_t* envelope, uint16_t addr_of_interest, uint_fast8_t endpoint,
...
#define ZDO_DEVICE_ANNCE_CLUSTER(handler, context)                                                                     \
    {ZDO_DEVICE_ANNCE, handler, context, WPAN_CLUST_FLAG_OUTPUT | WPAN_CLUST_FLAG_NOT_ZCL}
...
typedef XBEE_PACKED(zdo_mgmt_leave_rsp_t, { uint8_t status; }) zdo_mgmt_leave_rsp_t;
...
/**
   ...
   @retval  0     request sent
*/
int zdo_send_descriptor_req(wpan_envelope_t* envelope, uint16_t cluster, uint16_t addr_of_interest,
                            wpan_response_fn callback, const void FAR* context);

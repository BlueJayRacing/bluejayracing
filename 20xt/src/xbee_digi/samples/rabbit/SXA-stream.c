...
int send_data(const sxa_node_t FAR* sxa, void FAR* data, uint16_t length)
{
    ...
    wpan_envelope_create(&env, &my_xbee.wpan_dev, &sxa->id.ieee_addr_be, sxa->id.network_addr);
    env.payload = data;
    env.length  = length;
...
int send_string(const sxa_node_t FAR* sxa, char FAR* str) { return send_data(sxa, str, strlen(str)); }
...
const wpan_cluster_table_entry_t digi_data_clusters[] = {
    ...
    {DIGI_CLUST_SERIAL, xbee_transparent_rx, NULL, WPAN_CLUST_FLAG_INOUT | WPAN_CLUST_FLAG_NOT_ZCL},
...
char qbf[] = {"The quick brown fox jumps over the lazy dog. 012345678901234567\n"
        ...
        if (!strncmpi(cmdstr, "menu", 4)) {
            ...
            if (!have_target) {
                ...
                if (slash) {
                    ...
                    if (chunksize > sizeof(qbf) || chunksize < 1)
                        chunksize = sizeof(qbf);
                } else {
                    chunksize = 64;
                }
                msgsent = 0;
            ...
            process_command_remote(sxa_xbee(sxa), cmdstr, have_target ? &target_ieee : NULL);
        } else if (have_target) {
...
int my_handle_transmit_status(xbee_dev_t* xbee, const void FAR* payload, uint16_t length, void FAR* context)
{
    ...
    printf("TS: id 0x%02x to 0x%04x retries=%d del=0x%02x=%s disc=0x%02x=%s\n", frame->frame_id,
           ...
           xbee_delivery_status_str(frame->delivery), frame->discovery, xbee_discovery_status_str(frame->discovery));

    return 0;
}

int my_modem_status(xbee_dev_t* xbee, const void FAR* payload, uint16_t length, void FAR* context)
{

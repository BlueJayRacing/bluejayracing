...
__nodebug void serial_bypass(void)
{
...
#asm __nodebug
    ...
    disable interrupts
        ...
        ;
    change Tx pin to host to normal digital output ld hl, PCFR ioi res _HOST_DRIVE_TXD, (hl)

        ;
    change Tx pin to XBee to normal digital output ld hl, _XBEE_SER_FR ioi res _XBEE_DRIVE_TXD,
        (hl)

            ld hl,
        PCDR;
    Serial A(host) Tx and Rx ld ix,
        _XBEE_SER_DR

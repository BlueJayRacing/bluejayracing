...
char* get_file()
{
    ...
    file = win32_select_file("Select Firmware Image", "Firmware Images (*.abs.bin)\0*.abs.bin\0All Files (*.*)\0*.*\0");
    if (file != NULL) {
...
void parse_args(int argc, char* argv[])
{
    ...
    for (i = 1; i < argc; ++i) {
        if (strncmp(argv[i], "--profile=", 10) == 0) {
            dynamic_profile.profile_id = (uint16_t)strtoul(&argv[i][10], NULL, 16);
...
int main(int argc, char* argv[])
{
    ...
    while (1) {
...
#else
                    printf(" %" PRIu32 " bytes\r", UINT32_C(64) * pxbee_ota.xbxm.packet_num);
#endif
        ...
        if (linelen == -ENODATA || !strcmpi(cmdstr, "quit")) {
            ...
            printf("set password to [%.*s]\n", pxbee_ota.auth_length, pxbee_ota.auth_data);
            ...
            status = xbee_xmodem_set_source(&pxbee_ota.xbxm, xmodem_buffer, fw_read, fw_file);
            if (status) {

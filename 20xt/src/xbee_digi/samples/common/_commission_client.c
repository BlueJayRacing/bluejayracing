...
int commissioning_client(const wpan_envelope_t FAR* envelope, void FAR* context)
{
    const char* cmd = NULL;
    zcl_command_t zcl;
    int err;
    ...
    if (err) {
        ...
        if (ZCL_CMD_MATCH(&zcl.frame_control, GENERAL, SERVER_TO_CLIENT, CLUSTER)) {
            switch (zcl.command) {
            ...
            if (cmd != NULL && zcl.length == 1) {
                printf("\t%s: Status = %s\n", cmd, zcl_status_text(*(const uint8_t FAR*)zcl.zcl_payload));
...
const wpan_cluster_table_entry_t zcl_cluster_table[] = {
    ...
    {ZCL_CLUST_COMMISSIONING, &commissioning_client, NULL, WPAN_CLUST_FLAG_CLIENT},
...
int xbee_found(wpan_conversation_t FAR* conversation, const wpan_envelope_t FAR* envelope)
{
    ...
    if (i >= 0) {
        ...
        target_list[i].endpoint = reply_envelope.dest_endpoint = match_response->endpoints[0];

...
void transparent_dump(const addr64 FAR* ieee, const void FAR* payload, uint16_t length)
{
...
{
    transparent_dump(&envelope->ieee_address, envelope->payload, envelope->length);

    return 0;
}

// function receiving data on transparent serial cluster when ATAO=0
int receive_handler(xbee_dev_t* xbee, const void FAR* raw, uint16_t length, void FAR* context)
{
    const xbee_frame_receive_t FAR* rx_frame = raw;

    if (length >= offsetof(xbee_frame_receive_t, payload)) {
        transparent_dump(&rx_frame->ieee_address, rx_frame->payload, length - offsetof(xbee_frame_receive_t, payload));
    }
...
int send_data(const xbee_node_id_t* node_id, void FAR* data, uint16_t length)
{
    ...
    wpan_envelope_create(&env, &my_xbee.wpan_dev, &node_id->ieee_addr_be, node_id->network_addr);
    env.payload = data;
    env.length  = length;
...
int send_string(const xbee_node_id_t* node_id, char FAR* str) { return send_data(node_id, str, strlen(str)); }
...
const wpan_cluster_table_entry_t digi_data_clusters[] = {
    ...
    {DIGI_CLUST_SERIAL, transparent_rx, NULL, WPAN_CLUST_FLAG_INOUT | WPAN_CLUST_FLAG_NOT_ZCL},

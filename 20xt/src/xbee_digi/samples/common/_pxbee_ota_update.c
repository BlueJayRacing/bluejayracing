...
void print_target(int index)
{
    ...
    if (!addr64_is_zero(&t->ieee)) {
        printf("%2u: %08" PRIx32 "-%08" PRIx32 " 0x%02x %-32s %-16s\n", index, be32toh(t->ieee.l[0]),
               be32toh(t->ieee.l[1]), t->basic.app_ver, t->basic.model_id, t->basic.datecode);
    }
}
...
int basic_client(const wpan_envelope_t FAR* envelope, void FAR* context)
{
    zcl_command_t zcl;
...
const wpan_cluster_table_entry_t zcl_cluster_table[] = {
    {ZCL_CLUST_BASIC, &basic_client, zcl_basic_attribute_tree, WPAN_CLUST_FLAG_INOUT},
...
int pxbee_ota_error(const wpan_envelope_t FAR* envelope, void FAR* context)
{
...
wpan_cluster_table_entry_t digi_data_clusters[] = {
    ...
    PXBEE_OTA_CMD_CLIENT_CLUST_ENTRY(pxbee_ota_error, NULL, WPAN_CLUST_FLAG_NONE),
...
int pxbee_ota_find_devices(wpan_dev_t* dev, wpan_response_fn callback, const void FAR* context)
{
    static uint16_t clusters[] = {DIGI_CLUST_PROG_XBEE_OTA_UPD, WPAN_CLUSTER_END_OF_LIST};

    // find devices with OTA
    return zdo_send_match_desc(dev, clusters, WPAN_PROFILE_DIGI, callback, context);
}

// This should be the ZDO Match Descriptor response to our search for
// an XBee with the OTA update cluster.
int xbee_found(wpan_conversation_t FAR* conversation, const wpan_envelope_t FAR* envelope)
{
    int i;

    const uint16_t attributes[]         = {ZCL_BASIC_ATTR_APP_VERSION, ZCL_BASIC_ATTR_STACK_VERSION,
                                           ZCL_BASIC_ATTR_MODEL_IDENTIFIER, ZCL_BASIC_ATTR_DATE_CODE,
                                           ZCL_ATTRIBUTE_END_OF_LIST};
    const zcl_client_read_t client_read = {&sample_endpoints.zcl, ZCL_MFG_NONE, ZCL_CLUST_BASIC, attributes};

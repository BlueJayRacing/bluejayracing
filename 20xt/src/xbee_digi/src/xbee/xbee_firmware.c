...
*/
_xbee_firmware_debug int xbee_fw_install_init(xbee_dev_t* xbee, const wpan_address_t FAR* target,
...
// resetting XBee module.
    ...
    case XBEE_FW_STATE_XMODEM_WAIT:
        if (xbee_ser_getchar(serport) == 'C') {
            ...
            xbee_xmodem_set_source(&source->u.xbxm, source->buffer, source->read, source->context);
            result        = xbee_xmodem_tx_init(&source->u.xbxm, XBEE_XMODEM_FLAG_128 | XBEE_XMODEM_FLAG_FORCE_CRC);
            source->state = result ? XBEE_FW_STATE_FAILURE : XBEE_FW_STATE_XMODEM_SEND;
            return result;
        }
        break;
...
*/
_xbee_firmware_debug char FAR* xbee_fw_status_ebl(xbee_fw_source_t* source, char FAR* buffer)
{
    ...
    case XBEE_FW_STATE_XMODEM_SEND:
        ...
        case XBEE_XMODEM_STATE_SENDING:
            sprintf(buffer, "XMODEM Packet %" PRIu16 ": data.", source->u.xbxm.packet_num);
            return buffer;

        case XBEE_XMODEM_STATE_WAIT_ACK:
            sprintf(buffer, "XMODEM Packet %" PRIu16 ": data, checksum, ACK.", source->u.xbxm.packet_num);
            return buffer;
...
#define XBEE_OEM_OFS_MODULE_ID                                                                                         \
    10 // 8-bit value, must match upper
       // byte of XBee's ATHV response
#define XBEE_OEM_OFS_SW_COMPAT_LEVEL                                                                                   \
    11 // 8-bit value, must match XBee's
       // AT%C response
#define XBEE_OEM_OFS_FW_VERSION                                                                                        \
    12 // variable-length string?  Must
...
_xbee_firmware_debug int _xbee_oem_verify(xbee_fw_source_t* source)
{
    ...
    if (source->seek(source->context, 0) != 0 ||
        source->read(source->context, &oem_header, sizeof oem_header) != sizeof oem_header) {
        return -EIO;
    }
...
// Put the radio in command mode.
    ...
    case XBEE_FW_STATE_PARSE_RESPONSE:
        retval = xbee_atmode_read_response(xbee, source->buffer, sizeof(source->buffer), NULL);
        if (retval == -EAGAIN) {
            break;
        } else if (retval < 0) {
#ifdef XBEE_FIRMWARE_VERBOSE
    ...
    case XBEE_FW_STATE_CMD_COMPAT:
        ...
        if (strcmp(source->buffer, "ERROR")) { // response to AT%C is not "ERROR"
    ...
    case XBEE_FW_STATE_CMD_SL:
        ...
        sprintf(response, "%%P%04X", (xbee_fw_hex2word(source->buffer) + 0xDB8A) & 0x3fff);
        _xbee_fw_send_request(source, response, XBEE_FW_STATE_CMD_PROG);
        break;

    case XBEE_FW_STATE_CMD_PROG:
        if (strcmp(source->buffer, "OK")) { // Response to AT%Pxxxx was not "OK", so trigger a firmware
...
*/
_xbee_firmware_debug char FAR* xbee_fw_status_oem(xbee_fw_source_t* source, char FAR* buffer)
{
    if (!source || !source->xbee) {
        return "Invalid source.";
    }
    ...
    case XBEE_FW_STATE_CMD_PENDING:
        sprintf(buffer, "Trying to enter command mode at %skbps.", source->tries ? "9.6" : "115");
        return buffer;
    ...
    case XBEE_FW_STATE_RX_BLOCK:
    case XBEE_FW_STATE_RX_FAIL:
        sprintf(buffer, "Sending firmware (%" PRIu32 "/%" PRIu32 ").", source->u.oem.block_offset,
                source->u.oem.firmware_length);
        return buffer;
...
*/
_xbee_firmware_debug int xbee_fw_buffer_init(xbee_fw_buffer_t* fw, uint32_t length, const char FAR* address)
{

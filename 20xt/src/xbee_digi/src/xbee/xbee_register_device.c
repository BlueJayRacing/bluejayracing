...
int xbee_register_device(xbee_dev_t* xbee, addr64 device_addr_be, const void* key, unsigned key_len)
{
    ...
    if (xbee == NULL || (key_len > XBEE_ZIGBEE_LINK_KEY_MAXLEN && key_len != XBEE_ZIGBEE_INSTALL_KEY_LEN) ||
        (key == NULL && key_len > 0)) {
        return -EINVAL;
    }
    ...
    frame.options     = key_len == 18 ? XBEE_REG_DEV_OPT_INSTALL_CODE : XBEE_REG_DEV_OPT_LINK_KEY;

    int retval = xbee_frame_write(xbee, &frame, sizeof(frame), key, key_len, XBEE_WRITE_FLAG_NONE);
...
{
    switch (status) {
    case XBEE_REG_DEV_STATUS_SUCCESS:
        return "success";
    case XBEE_REG_DEV_STATUS_KEY_TOO_LONG:
        return "key too long";
    case XBEE_REG_DEV_STATUS_ADDR_NOT_FOUND:
        return "address not found";
    case XBEE_REG_DEV_STATUS_KEY_INVALID:
        return "key invalid";
    case XBEE_REG_DEV_STATUS_ADDR_INVALID:
        return "address invalid";
    case XBEE_REG_DEV_STATUS_KEY_TABLE_FULL:
        return "key table full";
    case XBEE_REG_DEV_STATUS_BAD_INSTALL_CRC:
        return "bad install code crc";
    case XBEE_REG_DEV_STATUS_KEY_NOT_FOUND:
        return "key not found";
    }

    snprintf(buffer, XBEE_REGISTER_DEVICE_STR_BUF_SIZE, "unknown 0x%02X", status);
    return buffer;
}

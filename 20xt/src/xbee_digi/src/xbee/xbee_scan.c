        ...
        if (XBEE_AT_RESP_STATUS(resp->header.status) == XBEE_AT_RESP_SUCCESS) {
            ...
            if (scan_length < 1) {
                ...
                case XBEE_SCAN_TYPE_WIFI: {
                    int ssid_length = scan_length - offsetof(xbee_scan_response_t, wifi.ssid);
                    if (ssid_length < 1) {
                        printf("Short (%u-byte) response:\n", scan_length);
                        hexdump = TRUE;
                    } else {
                        printf("CH%2u  Enc: %-4s  LM:%3udBm  ID:[%.*s]\n", atas->wifi.channel,
                               xbee_wifi_encryption_name(atas->wifi.security_type), atas->wifi.link_margin, ssid_length,
                               atas->wifi.ssid);
                    }
                    break;
                }
                ...
                case XBEE_SCAN_TYPE_ZIGBEE:
                    if (scan_length < sizeof atas->zigbee) {
                        ...
                        printf("CH%2u PAN:0x%04X %" PRIsFAR " J%u SP%u LQI:%3u RSSI:%d\n", atas->zigbee.channel,
                               be16toh(atas->zigbee.pan_be), addr64_format(buffer, &atas->zigbee.extended_pan_be),
                               atas->zigbee.allow_join, atas->zigbee.stack_profile, atas->zigbee.lqi,
                               atas->zigbee.rssi);
                    }
                    break;
                default:

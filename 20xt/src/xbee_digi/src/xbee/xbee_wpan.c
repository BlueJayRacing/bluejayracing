...
*/
xbee_wpan_debug int _xbee_handle_receive_explicit(xbee_dev_t* xbee, const void FAR* raw, uint16_t length,
                                                  void FAR* context)
{
...
/**
   ...
   function directly.
*/
xbee_wpan_debug int _xbee_endpoint_send(const wpan_envelope_t FAR* envelope, uint16_t flags)
{
    xbee_dev_t* xbee;
    ...
    header.options            = (flags & WPAN_SEND_FLAG_ENCRYPTED) ? XBEE_TX_OPT_APS_ENCRYPT : 0;

#ifdef XBEE_WPAN_VERBOSE
    printf("%s: %u bytes to 0x%04x "
           "(ep=%02x->%02x clust=%04x prof=%04x opt=%02x)\n",
           __FUNCTION__, envelope->length, envelope->network_address, envelope->source_endpoint,
           envelope->dest_endpoint, envelope->cluster_id, envelope->profile_id, header.options);
#endif

    error = xbee_frame_write(xbee, &header, sizeof(header), envelope->payload, envelope->length, 0);
...
*/
xbee_wpan_debug int _xbee_wpan_tick(wpan_dev_t* dev) { return xbee_dev_tick((xbee_dev_t*)dev); }
...
*/
xbee_wpan_debug int xbee_wpan_init(xbee_dev_t* xbee, const wpan_endpoint_table_entry_t* ep_table)
{
...
xbee_wpan_debug int _xbee_handle_transmit_status(xbee_dev_t* xbee, const void FAR* payload, uint16_t length,
                                                 void FAR* context)
{
...
/*** EndHeader */
int xbee_frame_dump_transmit_status(xbee_dev_t* xbee, const void FAR* payload, uint16_t length, void FAR* context)
{
    ...
    printf("%s: id 0x%02x to 0x%04x retries=%d del=0x%02x disc=0x%02x\n", __FUNCTION__, frame->frame_id,
           be16toh(frame->network_address_be), frame->retries, frame->delivery, frame->discovery);

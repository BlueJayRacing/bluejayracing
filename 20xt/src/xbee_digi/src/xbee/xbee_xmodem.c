...
_xmodem_debug int _xbee_xmodem_ser_read(void FAR* context, void FAR* buffer, int16_t bytes)
{
    return xbee_ser_read(CAST_FAR_TO_NEAR(context), buffer, bytes);
}
_xmodem_debug int _xbee_xmodem_ser_write(void FAR* context, const void FAR* buffer, int16_t bytes)
{
...
_xmodem_debug int xbee_xmodem_use_serport(xbee_xmodem_state_t* xbxm, xbee_serial_t* serport)
{
    return xbee_xmodem_set_stream(xbxm, _xbee_xmodem_ser_read, _xbee_xmodem_ser_write, serport);
}

/*** BeginHeader xbee_xmodem_set_stream */
/*** EndHeader */
// documented in xbee/xmodem.h
_xmodem_debug int xbee_xmodem_set_stream(xbee_xmodem_state_t* xbxm, xbee_xmodem_read_fn read,
                                         xbee_xmodem_write_fn write, const void FAR* context)
{
...
/*** EndHeader */
// documented in xbee/xmodem.h
_xmodem_debug int xbee_xmodem_set_source(xbee_xmodem_state_t* xbxm, void FAR* buffer, xbee_xmodem_read_fn read,
...
*/
_xmodem_debug int _xbee_xmodem_putchar(xbee_xmodem_state_t* xbxm, uint8_t ch)
{
    int retval;
...
/*** EndHeader */
// documented in xbee/xmodem.h
_xmodem_debug int xbee_xmodem_tx_init(xbee_xmodem_state_t* xbxm, uint16_t flags)
{
    if (!xbxm) {
        return -EINVAL;
    }
...
_xmodem_debug int _assemble_packet(xbee_xmodem_state_t* xbxm, uint16_t block_size)
{
    ...
    xbxm->tries     = 3;
    xbxm->buffer[0] = (xbxm->flags & XBEE_XMODEM_FLAG_1K) ? XMODEM_STX : XMODEM_SOH;
...
_xmodem_debug int xbee_xmodem_tx_tick(xbee_xmodem_state_t* xbxm)
{
...
#ifdef XBEE_XMODEM_VERBOSE
        printf("%s: starting XMODEM%s\n", __FUNCTION__, (ch == XMODEM_CRC) ? "-CRC" : "");
#endif
    ...
    case XBEE_XMODEM_STATE_RESEND:
        ...
        if (xbxm->flags & (XBEE_XMODEM_FLAG_BAD_CRC | XBEE_XMODEM_FLAG_RESTORE_CRC)) {
            bool_t restore;
            ...
            xbxm->flags &= ~(XBEE_XMODEM_FLAG_BAD_CRC | XBEE_XMODEM_FLAG_RESTORE_CRC);
            if (restore) {
...
#endif
        {
            sent = xbxm->stream.write(xbxm->stream.context, &xbxm->buffer[xbxm->offset], packet_size - xbxm->offset);
        }

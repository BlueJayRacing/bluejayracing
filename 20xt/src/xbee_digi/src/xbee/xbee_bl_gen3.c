...
void xbee_gen3_dump_extended_ver(const xbee_gen3_extended_ver_t* ver)
{
    ...
    printf("ATHV:%04X  AT%%R:%04X  AT%%C:%02X  ATHS:%04X  MAC:%s\n", le16toh(ver->hw_version_le),
           le16toh(ver->hw_revision_le), ver->hw_compatibility, le16toh(ver->hw_series_le),
...
uint16_t xbee_bl_gen3_install_state(xbee_gen3_update_t* source) { return (source->page_num << 6) | source->state; }
...
static int _load_page(xbee_gen3_update_t* source)
{
    ...
    while (1) {
        ...
        bytes_read = source->read(source->context, source->buffer, XBEE_GEN3_UPLOAD_PAGE_SIZE);
        ...
        if (source->flags & XBEE_GEN3_BL_FLAG_EOF) {
            ...
            memset(&source->buffer[bytes_read], 0xFF, XBEE_GEN3_UPLOAD_PAGE_SIZE - bytes_read);
        }
...
static void _send_page(xbee_gen3_update_t* source)
{
    ...
    if (source->page_offset < 0 && tx_free >= 3) {
        ...
        xbee_ser_write(serport, &source->buffer[source->page_offset], to_send);
        ...
        if (source->flags & XBEE_GEN3_BL_FLAG_PROTO1) { // CRC16
            uint16_t crc16 = crc16buypass_byte(0, source->buffer, XBEE_GEN3_UPLOAD_PAGE_SIZE);
...
int xbee_bl_gen3_install_tick(xbee_gen3_update_t* source)
{
...
#ifdef XBEE_BL_GEN3_VERBOSE
                printf("%s: bl version 0x%04X\n", __FUNCTION__, source->bl_version);
#endif
            }
...
#endif
            xbee_ser_baudrate(serport, (uint32_t)strtoul(source->buffer, NULL, 10));
    ...
    case XBEE_GEN3_STATE_EXT_VERSION:
        if (xbee_ser_rx_used(serport) >= sizeof(xbee_gen3_extended_ver_t)) {
            xbee_ser_read(serport, &source->ext_ver, sizeof(xbee_gen3_extended_ver_t));
#ifdef XBEE_BL_GEN3_VERBOSE
            xbee_gen3_dump_extended_ver(&source->ext_ver);
#endif
            if ((source->flags & XBEE_GEN3_BL_FLAG_QUERY_ONLY) || source->read == NULL) {
...
const char* xbee_bl_gen3_install_status(xbee_gen3_update_t* source, char buffer[XBEE_GEN3_STATUS_BUF_SIZE])
{
    ...
    case XBEE_GEN3_STATE_BAUD_RATE: // parse 'R'/'X', change baud, send 'V'
        sprintf(buffer, "BL protocol %u.", (source->flags & XBEE_GEN3_BL_FLAG_PROTO1) ? 1 : 0);
        return buffer;

    case XBEE_GEN3_STATE_EXT_VERSION: // parse 'V' response, send 'I'
        sprintf(buffer, "Switched to %" PRIu32 " baud.", source->xbee->serport.baudrate);
        return buffer;

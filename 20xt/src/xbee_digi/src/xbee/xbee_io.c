...
*/
_xbee_io_debug int xbee_io_response_parse(xbee_io_t FAR* parsed, const void FAR* source)
{
...
/*** EndHeader */
_xbee_io_debug char* _xbee_mask_to_binary(char* buf, uint16_t mask, char* digits, int maxbit)
{
    uint16_t m;
    char* p = buf;
...
_xbee_io_debug char* _xbee_2masks_to_quaternary(char* buf, uint16_t mask_lo, uint16_t mask_hi, char* digits, int maxbit)
{
    uint16_t m;
    char* p = buf;
...
*/
_xbee_io_debug void xbee_io_response_dump(const xbee_io_t FAR* io)
{
    uint16_t m;
    int i;
    char buf[20];
    ...
    if (io->analog_enabled) {
        printf("Analog state:\n");
        for (m = 1, i = 0; i < _TABLE_ENTRIES(io->adc_sample); m <<= 1, ++i) {
...
_xbee_io_debug int xbee_io_get_digital_input(const xbee_io_t FAR* io, uint_fast8_t index)
{
    uint16_t m = 1u << index;
...
_xbee_io_debug int xbee_io_get_digital_output(const xbee_io_t FAR* io, uint_fast8_t index)
{
    uint16_t m = 1u << index;
...
_xbee_io_debug int16_t xbee_io_get_analog_input(const xbee_io_t FAR* io, uint_fast8_t index)
{
    uint16_t m = 1u << index;
...
_xbee_io_debug int xbee_io_set_digital_output(xbee_dev_t* xbee, xbee_io_t FAR* io, uint_fast8_t index,
                                              ...
                                              const wpan_address_t FAR* address)
{
    ...
    if (!io || !xbee || !m || (state != XBEE_IO_SET_LOW && state != XBEE_IO_SET_HIGH)) {
        return -EINVAL;
    }
...
_xbee_io_debug int xbee_io_configure(xbee_dev_t* xbee, xbee_io_t FAR* io, uint_fast8_t index, enum xbee_io_type type,
                                     const wpan_address_t FAR* address)
{
    uint16_t m = 1u << index;
    uint16_t not_m;
    ...
    if (type > XBEE_IO_TYPE_TXEN_ACTIVE_HIGH || (type > XBEE_IO_TYPE_DIGITAL_OUTPUT_HIGH && index != 7) ||
        (type == XBEE_IO_TYPE_ANALOG_INPUT && index >= 4) ||
        (type == XBEE_IO_TYPE_SPECIAL && (index >= 11 || (index >= 1 && index < 5)))) {
        return -EPERM;
    }
...
_xbee_io_debug int xbee_io_set_options(xbee_dev_t* xbee, xbee_io_t FAR* io, uint16_t sample_rate, uint16_t change_mask,
                                       const wpan_address_t FAR* address)
{
    int16_t request;
...
const uint16_t _xbee_pullup_atcmd_to_iomask[16] = {
    ...
    // unimplemented bits
    1u << 4, 1u << 3, 1u << 2,  1u << 1,  1u << 0,  1u << 6, 1u << 8,
    0, // DIN/CONFIG is not a configurable I/O
    1u << 5, 1u << 9, 1u << 12, 1u << 10, 1u << 11, 1u << 7, 0,       0};

_xbee_io_debug void _xbee_io_query_handle_pr(const xbee_cmd_response_t FAR* response,
...
_xbee_io_debug void _xbee_io_query_handle_end(const xbee_cmd_response_t FAR* response,
                                              const struct xbee_atcmd_reg_t FAR* reg, void FAR* base)
{
    xbee_io_t FAR* io = base;
    uint_fast8_t i;
...
const xbee_atcmd_reg_t _xbee_io_query_regs[] = {XBEE_ATCMD_REG('D', '0', XBEE_CLT_COPY, xbee_io_t, config[0]),
...
_xbee_io_debug int xbee_io_query(xbee_dev_t* xbee, xbee_io_t FAR* io, const wpan_address_t FAR* address)
{
    if (!xbee || !io) {
        return -EINVAL;
    }
    ...
    return xbee_cmd_list_execute(xbee, &io->clc, _xbee_io_query_regs, io, address);
}
...
/**
   ...
   @retval  -EIO        halted, but query may not have completed
                           (unexpected response)
*/
_xbee_io_debug int xbee_io_query_status(xbee_io_t FAR* io)
{
    if (!io) {
        return -EINVAL;
    }

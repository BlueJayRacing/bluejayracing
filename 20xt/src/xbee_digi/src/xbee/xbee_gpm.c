...
int xbee_gpm_envelope_create(wpan_envelope_t FAR* envelope, wpan_dev_t* dev, const addr64 FAR* ieee)
{
    ...
    envelope->source_endpoint = envelope->dest_endpoint = WPAN_ENDPOINT_DIGI_DEVICE;
...
{
    return _xbee_gpm_parameterless_command(envelope, XBEE_GPM_CMD_PLATFORM_INFO_REQ);
}
...
int _xbee_gpm_write(const wpan_envelope_t* envelope, uint16_t block_num, uint16_t byte_offset, uint16_t byte_count,
                    const void FAR* data, bool_t erase_first)
{
    wpan_envelope_t gpm_envelope;
    XBEE_PACKED(, {
        xbee_gpm_request_header_t header;
    ...
    frame.header.cmd_id         = erase_first ? XBEE_GPM_CMD_ERASE_THEN_WRITE_REQ : XBEE_GPM_CMD_WRITE_REQ;
...
int xbee_gpm_write(const wpan_envelope_t* envelope, uint16_t block_num, uint16_t byte_offset, uint16_t byte_count,
                   const void FAR* data)
{
    return _xbee_gpm_write(envelope, block_num, byte_offset, byte_count, data, FALSE);
}

int xbee_gpm_erase_then_write(const wpan_envelope_t* envelope, uint16_t block_num, uint16_t byte_offset,
                              uint16_t byte_count, const void FAR* data)
{
    return _xbee_gpm_write(envelope, block_num, byte_offset, byte_count, data, TRUE);
}
...
{
    return _xbee_gpm_parameterless_command(envelope, XBEE_GPM_CMD_FIRMWARE_VERIFY_REQ);
}

int xbee_gpm_firmware_install(const wpan_envelope_t* envelope)
{
    return _xbee_gpm_parameterless_command(envelope, XBEE_GPM_CMD_FIRMWARE_INSTALL_REQ);
}

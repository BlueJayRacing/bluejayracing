...
int xbee_commissioning_init(zcl_comm_state_t* comm, xbee_dev_t* xbee)
{
    ...
    memset(&comm->flags, 0, sizeof(zcl_comm_state_t) - offsetof(zcl_comm_state_t, flags));
    ...
    switch ((uint16_t)xbee->firmware_version & (XBEE_PROTOCOL_MASK | XBEE_NODETYPE_MASK)) {
    case XBEE_PROTOCOL_ZB_S2C:                       // 40xx
    case XBEE_PROTOCOL_SE_S2C | XBEE_NODETYPE_COORD: // 51xx
        // S2C hardware can be any node type
        comm->flags = ZCL_COMM_FLAG_COORDINATOR_OK | ZCL_COMM_FLAG_ROUTER_OK | ZCL_COMM_FLAG_ENDDEV_OK;
        break;
...
#define _set_param_bytes(req, data, len) (memcpy_htobe(buffer, data, len), xbee_cmd_set_param_bytes(req, buffer, len))
#define _hex_dump(data, len)             hex_dump(buffer, len, HEX_DUMP_FLAG_NONE)
#else
#define _set_param_bytes(req, data, len) xbee_cmd_set_param_bytes(req, data, len)
#define _hex_dump(data, len)             hex_dump(data, len, HEX_DUMP_FLAG_NONE)
#endif
...
#ifdef XBEE_COMMISSIONING_VERBOSE
    printf("%s: setting %s=0x%08x\n", __FUNCTION__, "DL", dl.startup_control);
#endif
    xbee_cmd_send(request);
...
#ifdef XBEE_COMMISSIONING_VERBOSE
    printf("%s: setting %s=%u\n", __FUNCTION__, "EO", sas->use_insecure_join);
#endif
    xbee_cmd_send(request);

    // if network key has changed, set it on the XBee
    // Note that we can only set NK on an XBee coordinator
    if (memcheck(sas->network_key_le, 0xFF, sizeof(sas->network_key_le)) != 0) {
    ...
    if (memcheck(sas->preconfig_link_key_le, 0xFF, sizeof(sas->preconfig_link_key_le)) != 0) {
        ...
        encrypt = (memcheck(sas->preconfig_link_key_le, 0, sizeof(sas->preconfig_link_key_le)) != 0);
        ...
        memset(sas->preconfig_link_key_le, 0xFF, sizeof(sas->preconfig_link_key_le));
    }

    xbee_cmd_set_command(request, "BH");
    xbee_cmd_set_param(request, sas->concentrator.radius);
#ifdef XBEE_COMMISSIONING_VERBOSE
    printf("%s: setting %s=%u\n", __FUNCTION__, "BH", sas->concentrator.radius);
#endif
    xbee_cmd_send(request);
...
#ifdef XBEE_COMMISSIONING_VERBOSE
    printf("%s: setting SC=0x%04X\n", __FUNCTION__, (uint16_t)(sas->channel_mask >> 11));
#endif
    xbee_cmd_send(request);
...
void xbee_commissioning_tick(xbee_dev_t* xbee, zcl_comm_state_t* comm)
{
    ...
    if ((comm->flags & ZCL_COMM_FLAG_DELAYED_RESTART) && (int32_t)(xbee_millisecond_timer() - comm->restart_ms) >= 0) {
...
// send an ATWR to commit these changes to the XBee
    ...
    case ZCL_COMM_STATE_JOINED:
        if (!WPAN_DEV_IS_JOINED(&xbee->wpan_dev)) {
            ...
            memset(&comm->sas.extended_panid, 0xFF, sizeof(comm->sas.extended_panid));

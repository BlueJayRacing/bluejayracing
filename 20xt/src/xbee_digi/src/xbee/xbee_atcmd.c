...
_xbee_atcmd_debug int xbee_cmd_tick(void)
{
    ...
    for (request = xbee_cmd_request_table, i = XBEE_CMD_REQUEST_TABLESIZE; i; ++request, --i) {
        if (request->device && XBEE_CHECK_TIMEOUT_SEC(request->timeout)) {
            ++count;
            expired.handle = ((XBEE_CMD_REQUEST_TABLESIZE - i) << 8) | request->sequence;
...
*/
_xbee_atcmd_debug int _xbee_cmd_issue_list(xbee_dev_t* xbee, xbee_command_list_context_t FAR* clc,
...
#ifdef XBEE_ATCMD_VERBOSE
    printf("%s: next command AT%c%c\n", __FUNCTION__, reg->command.str[0], reg->command.str[1]);
#endif
    if (response) {
    ...
    if (command == reg->command.w) {
        ...
        if ((response->flags & XBEE_CMD_RESP_MASK_STATUS) == XBEE_AT_RESP_SUCCESS) {
            ...
            case XBEE_CLT_COPY_PAD_LEFT:
                ...
                count = response->value_length < reg->bytes ? response->value_length : reg->bytes;
        ...
        if (response->value_length <= 4) {
            printf("%s: Unexpected response AT%.*" PRIsFAR " = 0x%0*" PRIX32 "\n", __FUNCTION__, 2,
                   response->command.str, response->value_length * 2, response->value);
        } else {
            printf("%s: Unexpected %d-byte response to AT%.*" PRIsFAR "\n", __FUNCTION__, response->value_length, 2,
...
*/
_xbee_atcmd_debug int xbee_cmd_list_execute(xbee_dev_t* xbee, xbee_command_list_context_t FAR* clc,
                                            const xbee_atcmd_reg_t FAR* list, void FAR* base,
...
*/
_xbee_atcmd_debug enum xbee_command_list_status(xbee_cmd_list_status)(xbee_command_list_context_t FAR* clc)
{
    return clc->status;
}
...
_xbee_atcmd_debug void _xbee_cmd_query_handle_eo(const xbee_cmd_response_t FAR* response,
                                                 const struct xbee_atcmd_reg_t FAR* reg, void FAR* base)
{
    ...
    if ((response->flags & XBEE_CMD_RESP_MASK_STATUS) == XBEE_AT_RESP_SUCCESS) {
        ...
        printf("%s: EO error 0x%02X\n", __FUNCTION__, response->flags & XBEE_CMD_RESP_MASK_STATUS);
    }
#endif
}
                                                  ...
                                                  const struct xbee_atcmd_reg_t FAR* reg, void FAR* base)
{
    ...
    if (!reg) {
        ...
        xbee->flags |= XBEE_DEV_FLAG_QUERY_ERROR;
#ifdef XBEE_ATCMD_VERBOSE
        printf("%s: AT%c%c timed out!\n", __FUNCTION__, response->command.str[0], response->command.str[1]);
#endif
    } else {
    ...
    if (xbee->flags & XBEE_DEV_FLAG_QUERY_ERROR) {
        if (WPAN_DEV_IS_JOINED(&xbee->wpan_dev) && xbee->wpan_dev.address.network == WPAN_NET_ADDR_UNDEFINED) {
...
#ifdef XBEE_ATCMD_VERBOSE
    printf("%s: HV=0x%x  VR=0x%" PRIx32 "  IEEE=%" PRIsFAR "  net=0x%04" PRIx16 "\n", __FUNCTION__,
           xbee->hardware_version, xbee->firmware_version, addr64_format(buffer, &xbee->wpan_dev.address.ieee),
...
*/
_xbee_atcmd_debug int xbee_cmd_query_device(xbee_dev_t* xbee, uint_fast8_t refresh)
{
    int error;
    ...
    error = xbee_cmd_list_execute(xbee, &_xbee_atcmd_query_regs_head, _xbee_atcmd_query_regs + refresh, xbee, NULL);

    // successfully started query
    if (error == 0) {
        xbee->flags |= XBEE_DEV_FLAG_QUERY_BEGIN | XBEE_DEV_FLAG_QUERY_INPROGRESS;
        xbee->flags &= ~(XBEE_DEV_FLAG_QUERY_ERROR | XBEE_DEV_FLAG_QUERY_DONE | XBEE_DEV_FLAG_QUERY_REFRESH);
    }
...
/**
   ...
   @see     xbee_cmd_init_device(), xbee_cmd_query_device(),
            ...
            xbee_cmd_send()
*/
_xbee_atcmd_debug int16_t xbee_cmd_create(xbee_dev_t* xbee, const char FAR command[3])
{
    ...
    _f_memset(&request->timeout, 0, sizeof(*request) - offsetof(xbee_cmd_request_t, timeout));
...
/**
   ...
   @see  xbee_cmd_create(), xbee_cmd_set_callback(), xbee_cmd_set_target(),
         ...
         xbee_cmd_set_param_str(), xbee_cmd_send()
*/
_xbee_atcmd_debug int xbee_cmd_set_command(int16_t handle, const char FAR command[3])
{
    xbee_cmd_request_t FAR* request;
...
*/
_xbee_atcmd_debug int xbee_cmd_set_callback(int16_t handle, xbee_cmd_callback_fn callback, void FAR* context)
{
    xbee_cmd_request_t FAR* request;
...
_xbee_atcmd_debug int xbee_cmd_set_target(int16_t handle, const addr64 FAR* ieee, uint16_t network_address)
{
...
*/
_xbee_atcmd_debug uint8_t _xbee_cmd_encode_param(void FAR* buffer, uint32_t value)
{
...
*/
_xbee_atcmd_debug int xbee_cmd_set_flags(int16_t handle, uint16_t flags)
{
    xbee_cmd_request_t FAR* request;
...
*/
_xbee_atcmd_debug int xbee_cmd_clear_flags(int16_t handle, uint16_t flags)
{
    xbee_cmd_request_t FAR* request;
...
/**
   ...
   @see  xbee_cmd_create(), xbee_cmd_set_command(), xbee_cmd_set_callback(),
         ...
         xbee_cmd_set_param_str(), xbee_cmd_send()
*/
_xbee_atcmd_debug int xbee_cmd_set_param(int16_t handle, uint32_t value)
{
    xbee_cmd_request_t FAR* request;
...
/**
   ...
   @see  xbee_cmd_create(), xbee_cmd_set_command(), xbee_cmd_set_callback(),
         ...
         xbee_cmd_send()
*/
_xbee_atcmd_debug int xbee_cmd_set_param_bytes(int16_t handle, const void FAR* data, uint8_t length)
{
    xbee_cmd_request_t FAR* request;

    if (length > XBEE_CMD_MAX_PARAM_LENGTH) {
#ifdef XBEE_ATCMD_VERBOSE
        printf("%s: ERROR -EMSGSIZE (length %d > %d)\n", __FUNCTION__, length, XBEE_CMD_MAX_PARAM_LENGTH);
#endif
        return -EMSGSIZE;
    }
...
*/
_xbee_atcmd_debug int xbee_cmd_set_param_str(int16_t handle, const char FAR* str)
{
...
_xbee_atcmd_debug int xbee_cmd_send(int16_t handle)
{
    ...
    if (request->flags & XBEE_CMD_FLAG_REMOTE) {
        ...
        header.remote.options =
            (request->flags & XBEE_CMD_FLAG_QUEUE_CHANGE) ? XBEE_REMOTE_AT_OPT_QUEUE : XBEE_REMOTE_AT_OPT_IMMEDIATE;
    ...
    {
        header.frame_type =
            (request->flags & XBEE_CMD_FLAG_QUEUE_CHANGE) ? XBEE_FRAME_LOCAL_AT_CMD_Q : XBEE_FRAME_LOCAL_AT_CMD;
    ...
    error = xbee_frame_write(request->device, &header, header_size, request->param, request->param_length,
                             XBEE_DEV_FLAG_NONE);
...
#else
                XBEE_SET_TIMEOUT_SEC(request->flags & XBEE_CMD_FLAG_REMOTE ? XBEE_CMD_REMOTE_TIMEOUT
                                                                           : XBEE_CMD_LOCAL_TIMEOUT);
#endif
...
*/
_xbee_atcmd_debug int _xbee_cmd_handle_response(xbee_dev_t* xbee, const void FAR* rawframe, uint16_t length,
                                                void FAR* context)
{
    ...
    case XBEE_FRAME_LOCAL_AT_RESPONSE:
        ...
        value_length = (uint_fast8_t)(length - offsetof(xbee_frame_local_at_resp_t, value));
    ...
    case XBEE_FRAME_REMOTE_AT_RESPONSE:
        ...
        value_length = (uint_fast8_t)(length - offsetof(xbee_frame_remote_at_resp_t, value));
...
*/
_xbee_atcmd_debug int _xbee_cmd_modem_status(xbee_dev_t* xbee, const void FAR* payload, uint16_t length,
                                             void FAR* context)
{
    uint8_t status;
    ...
    if (status == XBEE_MODEM_STATUS_KEY_ESTABLISHED || status == XBEE_MODEM_STATUS_JOINED ||
...
_xbee_atcmd_debug int xbee_cmd_simple(xbee_dev_t* xbee, const char FAR command[3], uint32_t value)
{
    ...
    request_size = sizeof(request.header) + _xbee_cmd_encode_param(request.param, value);

#ifdef XBEE_ATCMD_VERBOSE
    printf("%s: sending AT%.2s=%" PRIu32 " (0x%" PRIx32 "), frame ID 0\n", __FUNCTION__, command, value, value);
#endif

    return xbee_frame_write(xbee, &request, request_size, NULL, 0, XBEE_DEV_FLAG_NONE);
}
...
*/
_xbee_atcmd_debug int xbee_cmd_execute(xbee_dev_t* xbee, const char FAR command[3], const void FAR* data,
                                       uint8_t length)
{
    ...
    return xbee_frame_write(xbee, &request, sizeof(request), data, length, XBEE_DEV_FLAG_NONE);
}

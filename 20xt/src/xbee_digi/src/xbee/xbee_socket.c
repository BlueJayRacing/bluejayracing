...
static socket_t* _lookup_sock_t(xbee_sock_t sock)
{
    ...
    if (index < XBEE_SOCK_SOCKET_COUNT && socket_table[index].create_frame_id == create_frame_id) {
...
static socket_t* _lookup_socket_id(xbee_dev_t* xbee, uint8_t socket_id)
{
    ...
    for (s = &socket_table[0]; s < &socket_table[XBEE_SOCK_SOCKET_COUNT]; ++s) {
        if (s->type != XBEE_SOCK_SOCKET_TYPE_AVAILABLE && s->xbee == xbee && s->socket_id == socket_id) {
            return s;
        }
    }
...
static void _socket_free(socket_t* s) { memset(s, 0, sizeof(*s)); }
...
xbee_sock_t xbee_sock_create(xbee_dev_t* xbee, uint8_t protocol, xbee_sock_notify_fn notify_handler)
{
    ...
    retval           = xbee_frame_write(xbee, &frame, sizeof(frame), NULL, 0, XBEE_WRITE_FLAG_NONE);
...
{
    if (receive_handler == NULL || remote_port == 0 || (remote_addr == 0 && remote_host == NULL)) {
        return -EINVAL;
    }
    ...
    int retval = xbee_frame_write(s->xbee, &header, sizeof(header), payload, payload_len, XBEE_WRITE_FLAG_NONE);

    debug_printf("%s: connect socket 0x%02X frame 0x%02X result %d\n", __FUNCTION__, s->socket_id, header.frame_id,
                 retval);
...
*/
static int _common_bind_listen(xbee_sock_t socket, uint16_t local_port, int is_bind, socket_t** sock)
{
    socket_t* s = _lookup_sock_t(socket);
    if (s == NULL) {
        return -ENOENT;
    ...
    if (s->type > XBEE_SOCK_SOCKET_TYPE_CREATED || is_bind != (s->protocol == XBEE_SOCK_PROTOCOL_UDP)) {
        return -EPERM;
    }
    ...
    int retval = xbee_frame_write(s->xbee, &frame, sizeof(frame), NULL, 0, XBEE_WRITE_FLAG_NONE);

    debug_printf("%s: socket 0x%02X frame 0x%02X result %d\n", __FUNCTION__, s->socket_id, frame.frame_id, retval);
...
int xbee_sock_send(xbee_sock_t socket, uint8_t tx_options, const void* payload, size_t payload_len)
{
    ...
    debug_printf("%s: %u bytes in frame 0x%02X  sock 0x%02X  result %d\n", __FUNCTION__, (unsigned)payload_len,
                 header.frame_id, s->socket_id, retval);
                     ...
                     xbee_sock_option_resp_fn callback)
{
    ...
    int retval = xbee_frame_write(s->xbee, &header, sizeof(header), data, data_len, XBEE_WRITE_FLAG_NONE);

    debug_printf("%s: %u bytes in frame 0x%02X  sock 0x%02X  result %d\n", __FUNCTION__, (unsigned)data_len,
                 header.frame_id, s->socket_id, retval);
...
static int _sock_close_frame(xbee_dev_t* xbee, uint8_t socket_id)
{
    ...
    return xbee_frame_write(xbee, &frame, sizeof(frame), NULL, 0, XBEE_WRITE_FLAG_NONE);
}
...
static int _sock_close(socket_t* s)
{
    ...
    if (retval == 0) {
        ...
        if (s->notify_handler != NULL) {
            ...
            s->notify_handler(_xbee_sock_t(s), XBEE_FRAME_SOCK_STATE, XBEE_SOCK_STATE_TRANSPORT_CLOSED);
        }
        _socket_free(s);
    }
...
// API documented in xbee/socket.h
int xbee_sock_close(xbee_sock_t socket) { return _sock_close(_lookup_sock_t(socket)); }
...
int xbee_sock_close_all(xbee_dev_t* xbee)
{
    ...
    for (s = &socket_table[0]; s < &socket_table[XBEE_SOCK_SOCKET_COUNT]; ++s) {
        ...
        if (s->type >= XBEE_SOCK_SOCKET_TYPE_CREATED && (xbee == NULL || s->xbee == xbee)) {
...
*/
static int handle_tx_status(xbee_dev_t* xbee, const xbee_frame_tx_status_t FAR* frame)
{
    // Search socket table for matching socket and notify user.
    socket_t* s;
    for (s = &socket_table[0]; s < &socket_table[XBEE_SOCK_SOCKET_COUNT]; ++s) {
        if (s->xbee == xbee && s->tx_frame_id == frame->frame_id) {
            s->notify_handler(_xbee_sock_t(s), XBEE_FRAME_TX_STATUS, frame->delivery);
...
/**
    ...
    @retval     -ENOENT         Couldn't find matching entry in socket table.
*/
static int handle_create_resp(xbee_dev_t* xbee, const xbee_frame_sock_shared_resp_t FAR* frame)
{
    debug_printf("%s: type 0x%02X  frame 0x%02X  sock 0x%02X  status 0x%02X\n", __FUNCTION__, frame->frame_type,
                 frame->frame_id, frame->socket_id, frame->status);

    // match response based on frame_id
    socket_t* s;
    for (s = &socket_table[0]; s < &socket_table[XBEE_SOCK_SOCKET_COUNT]; ++s) {
        if (s->type == XBEE_SOCK_SOCKET_TYPE_PENDING && s->xbee == xbee && s->create_frame_id == frame->frame_id) {
            if (frame->status == XBEE_SOCK_STATUS_SUCCESS) {
                s->socket_id = frame->socket_id;
                s->type      = XBEE_SOCK_SOCKET_TYPE_CREATED;
            }

            s->notify_handler(_xbee_sock_t(s), XBEE_FRAME_SOCK_CREATE_RESP, frame->status);
...
/**
    ...
    @retval     -ENOENT         Couldn't find matching entry in socket table.
*/
static int handle_attach_resp(xbee_dev_t* xbee, const xbee_frame_sock_shared_resp_t FAR* frame)
{
    debug_printf("%s: type 0x%02X  frame 0x%02X  sock 0x%02X  status 0x%02X\n", __FUNCTION__, frame->frame_type,
                 frame->frame_id, frame->socket_id, frame->status);
...
*/
static int handle_option_resp(xbee_dev_t* xbee, const xbee_frame_sock_option_resp_t FAR* frame, uint16_t length)
{
    int16_t payload_len = length - offsetof(xbee_frame_sock_option_resp_t, option_data);

    debug_printf("%s: frame 0x%02X  sock 0x%02X  opt 0x%02X  status 0x%02X\n", __FUNCTION__, frame->frame_id,
                 frame->socket_id, frame->option_id, frame->status);
    ...
    if (s->option_handler != NULL) {
        const void* option_data = payload_len ? &frame->option_data[0] : NULL;
        s->option_handler(_xbee_sock_t(s), frame->option_id, frame->status, option_data, payload_len);
    }
...
*/
static int handle_ipv4_client(xbee_dev_t* xbee, const xbee_frame_sock_ipv4_client_t FAR* frame)
{
    debug_printf("%s: listen 0x%02X  new client 0x%02X from 0x%08X:%u\n", __FUNCTION__, frame->sock_listen_id,
                 frame->sock_client_id, be32toh(frame->remote_addr_be), be16toh(frame->remote_port_be));
    ...
    xbee_sock_receive_fn client_rx_handler = listen->handler.ipv4_client(
        _xbee_sock_t(listen), _xbee_sock_t(client), be32toh(frame->remote_addr_be), be16toh(frame->remote_port_be));
...
*/
static int _common_receive(xbee_dev_t* xbee, int16_t payload_len, uint8_t type_check, uint8_t socket_id,
                           socket_t** socket)
{
...
*/
static int handle_receive(xbee_dev_t* xbee, const xbee_frame_sock_receive_t FAR* frame, uint16_t length)
{
    int16_t payload_len = length - offsetof(xbee_frame_sock_receive_t, payload);

    socket_t* s;
    int retval = _common_receive(xbee, payload_len, XBEE_SOCK_SOCKET_TYPE_CONNECT, frame->socket_id, &s);

    if (retval == 0) {
        s->handler.receive(_xbee_sock_t(s), frame->status, frame->payload, payload_len);
    }
...
*/
static int handle_receive_from(xbee_dev_t* xbee, const xbee_frame_sock_receive_from_t FAR* frame, uint16_t length)
{
    int16_t payload_len = length - offsetof(xbee_frame_sock_receive_from_t, payload);

    socket_t* s;
    int retval = _common_receive(xbee, payload_len, XBEE_SOCK_SOCKET_TYPE_BIND, frame->socket_id, &s);

    if (retval == 0) {
        s->handler.receive_from(_xbee_sock_t(s), frame->status, be32toh(frame->remote_addr_be),
                                be16toh(frame->remote_port_be), frame->payload, payload_len);
    }
...
/**
    ...
    @retval     -ENOENT         Couldn't find matching entry in socket table.
*/
static int handle_sock_state(xbee_dev_t* xbee, const xbee_frame_sock_state_t FAR* frame)
{
    debug_printf("%s: socket 0x%02X  state 0x%02x\n", __FUNCTION__, frame->socket_id, frame->state);

...
#if defined(MBEDTLS_CIPHER_MODE_XTS)
static int mbedtls_aes_xts_decode_keys(const unsigned char* key, unsigned int keybits, const unsigned char** key1,
                                       unsigned int* key1bits, const unsigned char** key2, unsigned int* key2bits)
{
...
#define GET_UINT64_LE(n, b, i)                                                                                         \
    {                                                                                                                  \
        (n) = ((uint64_t)(b)[(i) + 7] << 56) | ((uint64_t)(b)[(i) + 6] << 48) | ((uint64_t)(b)[(i) + 5] << 40) |       \
              ((uint64_t)(b)[(i) + 4] << 32) | ((uint64_t)(b)[(i) + 3] << 24) | ((uint64_t)(b)[(i) + 2] << 16) |       \
              ((uint64_t)(b)[(i) + 1] << 8) | ((uint64_t)(b)[(i)]);                                                    \
    }
#endif
...
int mbedtls_aes_self_test(int verbose)
{
...
#endif
#if defined(MBEDTLS_CIPHER_MODE_CTR) || defined(MBEDTLS_CIPHER_MODE_CFB) || defined(MBEDTLS_CIPHER_MODE_OFB)
    size_t offset;
#endif
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (verbose != 0)
            mbedtls_printf("  AES-ECB-%3d (%s): ", keybits, (mode == MBEDTLS_AES_DECRYPT) ? "dec" : "enc");
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (verbose != 0)
            mbedtls_printf("  AES-CBC-%3d (%s): ", keybits, (mode == MBEDTLS_AES_DECRYPT) ? "dec" : "enc");
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (verbose != 0)
            mbedtls_printf("  AES-CFB128-%3d (%s): ", keybits, (mode == MBEDTLS_AES_DECRYPT) ? "dec" : "enc");
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (verbose != 0)
            mbedtls_printf("  AES-OFB-%3d (%s): ", keybits, (mode == MBEDTLS_AES_DECRYPT) ? "dec" : "enc");
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (verbose != 0)
            mbedtls_printf("  AES-CTR-128 (%s): ", (mode == MBEDTLS_AES_DECRYPT) ? "dec" : "enc");
...
#if defined(MBEDTLS_CIPHER_MODE_XTS)
    {
        ...
        for (i = 0; i < num_tests << 1; i++) {
            ...
            if (verbose != 0)
                mbedtls_printf("  AES-XTS-128 (%s): ", (mode == MBEDTLS_AES_DECRYPT) ? "dec" : "enc");

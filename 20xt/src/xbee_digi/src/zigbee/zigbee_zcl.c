...
const zcl_attribute_tree_t FAR zcl_attributes_none[1] = {{ZCL_MFG_NONE, NULL, NULL}};
...
*/
zigbee_zcl_debug int zcl_send_response(zcl_command_t* request, const void FAR* response, uint16_t length)
{
    int retval;
...
#ifdef ZIGBEE_ZCL_VERBOSE
        printf("%s: sending %d-byte response\n", __FUNCTION__, reply_envelope.length);
...
zigbee_zcl_debug const zcl_attribute_base_t FAR* zcl_attribute_get_next(const zcl_attribute_base_t FAR* entry)
{
    ...
    if (entry->flags & ZCL_ATTRIB_FLAG_FULL) {
        ...
        return (const zcl_attribute_base_t FAR*)((const zcl_attribute_full_t FAR*)entry + 1);
    }
...
*/
zigbee_zcl_debug int zcl_build_header(zcl_header_response_t* rsp, zcl_command_t* cmd)
{
...
zigbee_zcl_debug int zcl_default_response(zcl_command_t* request, uint8_t status)
{
    ...
    if (status == ZCL_STATUS_SUCCESS && (request->frame_control & ZCL_FRAME_DISABLE_DEF_RESP)) {
#ifdef ZIGBEE_ZCL_VERBOSE
    ...
    if (request->envelope->options & (WPAN_ENVELOPE_BROADCAST_ADDR | WPAN_ENVELOPE_BROADCAST_EP)) {
#ifdef ZIGBEE_ZCL_VERBOSE
        printf("%s: not responding to %s\n", __FUNCTION__, "broadcast message");
#endif
        return 0;
    }
    ...
    *start &= ~ZCL_FRAME_TYPE_MASK;
    ...
    return zcl_send_response(request, start, (uint8_t*)(&response + 1) - start);
}
...
*/
zigbee_zcl_debug int zcl_check_minmax(const zcl_attribute_base_t FAR* entry, const uint8_t FAR* buffer_le)
{
#ifdef ZIGBEE_ZCL_VERBOSE
    ...
    if (entry->flags & (ZCL_ATTRIB_FLAG_HAS_MIN | ZCL_ATTRIB_FLAG_HAS_MAX)) {
        ...
        memcpy_letoh((uint8_t*)&value + (4 - sizeof_type), buffer_le, (uint8_t)sizeof_type);
#endif
        ...
        if (typeinfo & ZCL_T_SIGNED) {
            ...
            if ((entry->flags & ZCL_ATTRIB_FLAG_HAS_MIN) && (full->min._signed > value._signed)) {
                retval = 1;
            } else if ((entry->flags & ZCL_ATTRIB_FLAG_HAS_MAX) && (full->max._signed < value._signed)) {
                retval = 2;
            }
        } else {
            if ((entry->flags & ZCL_ATTRIB_FLAG_HAS_MIN) && (full->min._unsigned > value._unsigned)) {
                retval = 1;
            } else if ((entry->flags & ZCL_ATTRIB_FLAG_HAS_MAX) && (full->max._unsigned < value._unsigned)) {
                retval = 2;
            }
        }
    }
        ...
        if (!entry) {
            ...
            if (sizeof_type >= 0 && write_rec->buflen < sizeof_type) {
                ...
                printf("%s: buffer too small for type 0x%02x (have %d, need %d)\n", __FUNCTION__, type,
                       write_rec->buflen, sizeof_type);
#endif
                write_rec->status = ZCL_STATUS_MALFORMED_COMMAND;
            }
        }
        // if successful up until now, verify min/max values
        if (write_rec->status == ZCL_STATUS_SUCCESS && zcl_check_minmax(entry, write_rec->buffer)) {
...
/**
   ...
   @return A 32-bit, host-byte-order version of the 24-bit, little-endian
            value passed in.
*/
zigbee_zcl_debug uint32_t zcl_convert_24bit(const void FAR* value_le, bool_t extend_sign)
{
    uint8_t result[4];
...
/**
   ...
   @retval  -EINVAL  invalid parameter passed to function
*/
zigbee_zcl_debug int zcl_decode_attribute(const zcl_attribute_base_t FAR* entry, zcl_attribute_write_rec_t* rec)
{
    int retval = 0;
    int sizeof_type;
    uint8_t FAR* p;
    ...
    case ZCL_TYPE_SIGNED_24BIT:
        if (assign) {
            ...
            ulong = zcl_convert_24bit(rec->buffer, entry->type == ZCL_TYPE_SIGNED_24BIT);
#ifdef ZIGBEE_ZCL_VERBOSE
...
// swap the byte order while copying bytes.
#if BYTE_ORDER == BIG_ENDIAN
                if (!(entry->flags & ZCL_ATTRIB_FLAG_RAW)) {
                    memcpy_letoh((void FAR*)entry->value, rec->buffer, (uint8_t)sizeof_type);
                } else
#endif
                {
                    // simple copy for raw attribute or little-endian device
                    _f_memcpy((void FAR*)entry->value, rec->buffer, sizeof_type);
                }
            }
            retval = sizeof_type;
        }
    ...
    case ZCL_TYPE_STRING_CHAR:
        ...
        if (length + 1 > rec->buflen) // +1 for length byte
        {
            ...
            if ((entry->flags & ZCL_ATTRIB_FLAG_FULL) && maxlength && length > maxlength) {
                newstatus = ZCL_STATUS_INVALID_VALUE;
            } else if (assign) {
...
zigbee_zcl_debug int _zcl_write_attributes(zcl_command_t* cmd)
{
    ...
    start_response          = (uint8_t*)&response + zcl_build_header(&response.header, cmd);
    ...
    */
    ...
    while (pass <= 1 && status_rec == response.status) {
        ...
        while (parse_record.buflen > 0) {
            ...
            parse_record.flags = pass ? ZCL_ATTR_WRITE_FLAG_ASSIGN : ZCL_ATTR_WRITE_FLAG_NONE;
...
zigbee_zcl_debug int _zcl_encode_struct_value(uint8_t FAR* buffer, int16_t bufsize, const zcl_struct_t FAR* zcl_struct)
{
    ...
    while (element_count--) {
        ...
        fake_attr.value                    = (const uint8_t FAR*)zcl_struct->base_address + elem->offset;
        encode = zcl_encode_attribute_value(&buffer[written], bufsize - written, &fake_attr);
        if (encode < 0) {
...
/**
   ...
   @retval  >=0   number of bytes written
*/
zigbee_zcl_debug int zcl_encode_attribute_value(uint8_t FAR* buffer, int16_t bufsize,
                                                const zcl_attribute_base_t FAR* entry)
{
    ...
    case ZCL_TYPE_LOGICAL_BOOLEAN:
        ...
        // ZCL_BOOL_FALSE
        *buffer++ = *(uint8_t FAR*)entry->value ? ZCL_BOOL_TRUE : ZCL_BOOL_FALSE;
        break;
...
zigbee_zcl_debug int _zcl_read_attributes(zcl_command_t* cmd)
{
    ...
    for (id_le = (uint16_t FAR*)cmd->zcl_payload; requests; --requests, ++id_le) {
    ...
    return zcl_send_response(cmd, start_response, end_response - start_response);
}
...
zigbee_zcl_debug int _zcl_discover_attributes(zcl_command_t* cmd)
{
    ...
    printf("%s: --- discover attributes, start @ 0x%04x, return %u max\n", __FUNCTION__,
           le16toh(discover->start_attrib_id_le), discover->max_return_count);
#endif
    ...
    return zcl_send_response(cmd, start_response, (uint8_t*)write_attrib - start_response);
}
...
*/
zigbee_zcl_debug int zcl_command_build(zcl_command_t* cmd, const wpan_envelope_t FAR* envelope,
    ...
    if (tree) {
        for (;;) {
            if (tree->manufacturer_id == command.mfg_code) {
                ...
                if ((command.frame_control & ZCL_FRAME_DIRECTION) == ZCL_FRAME_CLIENT_TO_SERVER) {
...
zigbee_zcl_debug void zcl_command_dump(const zcl_command_t* cmd)
{
    ...
    const uint8_t FAR* b;
    int length;
    const char* zcl_frame_type[]  = {"profile", "cluster", "reserved1", "reserved2"};
    const char* zcl_general_cmd[] = {"read_attrib",          "read_attrib_resp",        "write_attrib",
    ...
    printf("%s command, %s, ", zcl_frame_type[frame_control & ZCL_FRAME_TYPE_MASK],
           (frame_control & ZCL_FRAME_DIRECTION) == ZCL_FRAME_CLIENT_TO_SERVER ? "client-to-server"
                                                                               : "server-to-client");
    ...
    printf("\nsequence = 0x%02X, command = 0x%02X", cmd->sequence, cmd->command);
...
*/
zigbee_zcl_debug int zcl_invalid_cluster(const wpan_envelope_t FAR* envelope, wpan_ep_state_t FAR* ep_state)
{
    zcl_command_t zcl;
    int error;
...
zigbee_zcl_debug int zcl_invalid_command(const wpan_envelope_t FAR* envelope)
{
    ...
    error = zcl_command_build(&zcl, envelope, NULL);
    if (error == 0) {
        status = (zcl.frame_control & ZCL_FRAME_MFG_SPECIFIC) ? ZCL_STATUS_UNSUP_MANUF_GENERAL_COMMAND
                                                              : ZCL_STATUS_UNSUP_GENERAL_COMMAND;
...
*/
zigbee_zcl_debug int zcl_general_command(const wpan_envelope_t FAR* envelope, void FAR* context)
{
    zcl_command_t zcl;
    int conversation;
    int error;
    ...
    */
    ...
    case ZCL_CMD_WRITE_STRUCT_ATTRIB_RESP:
        conversation = wpan_conversation_response(NULL, zcl.sequence, envelope);
        if (conversation >= 0) {
...
zigbee_zcl_debug const char* zcl_status_text(uint_fast8_t status)
{
    ...
    case ZCL_STATUS_DUPLICATE_EXISTS:
    ...
    case ZCL_STATUS_INVALID_SELECTOR:
        return "Invalid Selector";
    case ZCL_STATUS_WRITE_ONLY:
        return "Write-only";
    ...
    case ZCL_STATUS_HARDWARE_FAILURE:
        return "Hardware Failure";
    case ZCL_STATUS_SOFTWARE_FAILURE:
        return "Software Failure";
    case ZCL_STATUS_CALIBRATION_ERROR:

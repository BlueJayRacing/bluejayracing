...
int _zcl_comm_startup_control_set(const zcl_attribute_full_t FAR* attribute, zcl_attribute_write_rec_t* rec)
{
    ...
    if (attribute != &zcl_comm_startup_attributes.startup_control) {
        ...
        case ZCL_COMM_STARTUP_FROM_SCRATCH:
            ...
            if (!(zcl_comm_state.flags & (ZCL_COMM_FLAG_ROUTER_OK | ZCL_COMM_FLAG_ENDDEV_OK))) {
                rec->status = ZCL_STATUS_INVALID_VALUE;
            }
            break;
        }

        if ((rec->flags & ZCL_ATTR_WRITE_FLAG_ASSIGN) && rec->status == ZCL_STATUS_SUCCESS) {
...
const zcl_comm_startup_attr_t zcl_comm_startup_attributes = {
    // id, flags, type, value
    {ZCL_COMM_ATTR_SHORT_ADDRESS, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_UNSIGNED_16BIT, &zcl_comm_state.sas.short_address},

    {ZCL_COMM_ATTR_EXTENDED_PANID, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_IEEE_ADDR, &zcl_comm_state.sas.extended_panid},

    {ZCL_COMM_ATTR_PANID, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_UNSIGNED_16BIT, &zcl_comm_state.sas.panid},

    {ZCL_COMM_ATTR_CHANNEL_MASK, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_BITMAP_32BIT, &zcl_comm_state.sas.channel_mask},

    {
        {ZCL_COMM_ATTR_STARTUP_CONTROL, ZCL_ATTRIB_FLAG_FULL, ZCL_TYPE_ENUM_8BIT, &zcl_comm_state.sas.startup_control},
    ...
    {
        ZCL_COMM_ATTR_TRUST_CTR_ADDR, ZCL_ATTRIB_FLAG_READONLY, ZCL_TYPE_IEEE_ADDR,
        WPAN_IEEE_ADDR_ALL_ZEROS // unspecified
    },

    {
        ZCL_COMM_ATTR_TRUST_CTR_MASTER_KEY, ZCL_ATTRIB_FLAG_WRITEONLY | ZCL_ATTRIB_FLAG_RAW, ZCL_TYPE_SECURITY_KEY,
        NULL // &zcl_comm_state.sas.trust_ctr_master_key
    },

    {ZCL_COMM_ATTR_NETWORK_KEY, ZCL_ATTRIB_FLAG_WRITEONLY | ZCL_ATTRIB_FLAG_RAW, ZCL_TYPE_SECURITY_KEY,
     &zcl_comm_state.sas.network_key_le},

    {ZCL_COMM_ATTR_USE_INSECURE_JOIN, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_LOGICAL_BOOLEAN,
     &zcl_comm_state.sas.use_insecure_join},

    {ZCL_COMM_ATTR_PRECONFIG_LINK_KEY, ZCL_ATTRIB_FLAG_WRITEONLY | ZCL_ATTRIB_FLAG_RAW, ZCL_TYPE_SECURITY_KEY,
     &zcl_comm_state.sas.preconfig_link_key_le},

    {ZCL_COMM_ATTR_NETWORK_KEY_SEQ_NUM, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_UNSIGNED_8BIT,
     &zcl_comm_state.sas.network_key_seq_num},

    {ZCL_COMM_ATTR_NETWORK_KEY_TYPE, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_ENUM_8BIT, &zcl_comm_state.sas.network_key_type},

    {ZCL_COMM_ATTR_NETWORK_MGR_ADDR, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_UNSIGNED_16BIT,
     &zcl_comm_state.sas.network_mgr_addr},

    {
        {ZCL_COMM_ATTR_SCAN_ATTEMPTS, ZCL_ATTRIB_FLAG_READONLY | ZCL_ATTRIB_FLAG_FULL, ZCL_TYPE_UNSIGNED_8BIT,
         &zcl_comm_state.sas.scan_attempts},
        {0x01},
        {0xFF},
        NULL,
        NULL // min, max, read, write
    },

    {
        {ZCL_COMM_ATTR_TIME_BTWN_SCANS, ZCL_ATTRIB_FLAG_READONLY | ZCL_ATTRIB_FLAG_FULL, ZCL_TYPE_UNSIGNED_16BIT,
    ...
    {
        {ZCL_COMM_ATTR_REJOIN_INTERVAL, ZCL_ATTRIB_FLAG_READONLY | ZCL_ATTRIB_FLAG_FULL, ZCL_TYPE_UNSIGNED_16BIT,
    ...
    {
        {ZCL_COMM_ATTR_MAX_REJOIN_INTERVAL, ZCL_ATTRIB_FLAG_READONLY | ZCL_ATTRIB_FLAG_FULL, ZCL_TYPE_UNSIGNED_16BIT,
    ...
    {{ZCL_COMM_ATTR_CONCENTRATOR_FLAG, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_LOGICAL_BOOLEAN,
      &zcl_comm_state.sas.concentrator.flag},

     {ZCL_COMM_ATTR_CONCENTRATOR_RADIUS, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_UNSIGNED_8BIT,
      &zcl_comm_state.sas.concentrator.radius},

     {ZCL_COMM_ATTR_CONCENTRATOR_DISC_TIME, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_UNSIGNED_8BIT,
...
int zcl_comm_response(const wpan_envelope_t FAR* envelope, uint_fast8_t status)
{
    ...
    reply.header.frame_control = ZCL_FRAME_SERVER_TO_CLIENT | ZCL_FRAME_DISABLE_DEF_RESP | ZCL_FRAME_TYPE_CLUSTER;
...
int _zcl_comm_command_build(wpan_envelope_t FAR* envelope, zcl_header_nomfg_t* header)
{
...
#endif
        ep = wpan_endpoint_of_cluster(envelope->dev, WPAN_APS_PROFILE_ANY, ZCL_CLUST_COMMISSIONING,
    ...
    header->frame_control =
        ZCL_FRAME_CLIENT_TO_SERVER | ZCL_FRAME_GENERAL | ZCL_FRAME_TYPE_CLUSTER | ZCL_FRAME_DISABLE_DEF_RESP;
...
bool_t zcl_comm_sas_is_valid(const zcl_comm_state_t FAR* comm)
{
    if (comm->sas.startup_control != ZCL_COMM_STARTUP_FROM_SCRATCH) {
        ...
        if (memcheck(&comm->sas.extended_panid, 0xFF, 8) == 0 || memcheck(&comm->sas.extended_panid, 0x00, 8) == 0) {
            return FALSE;
        }
    }
...
int zcl_comm_clust_handler(const wpan_envelope_t FAR* envelope, void FAR* context)
{
    ...
    if (envelope != NULL && ZCL_CMD_MATCH(envelope->payload, GENERAL, CLIENT_TO_SERVER, CLUSTER)) {
        const XBEE_PACKED(, {
        ...
        case ZCL_COMM_CMD_RESTART_DEVICE:
...
#endif
                return zcl_comm_response(envelope, ZCL_STATUS_INCONSISTENT_STARTUP_STATE);
            }

            // Set a timer for when we should perform the restart.
            delay = request->cmd.restart_dev.delay * UINT32_C(1000) + rand_range(request->cmd.restart_dev.jitter * 80);
#ifdef ZCL_COMMISSIONING_VERBOSE
            ...
            if ((request->cmd.restart_dev.options & ZCL_COMM_RESTART_OPT_MODE_MASK) ==
        ...
        case ZCL_COMM_CMD_RESET_STARTUP_PARAM:
            if (request->cmd.reset_startup.options & ZCL_COMM_RESET_OPT_CURRENT) {
#ifdef ZCL_COMMISSIONING_VERBOSE
                printf("%s: resetting to factory defaults\n", __FUNCTION__);
#endif
                // reset and install factory defaults
                comm->flags |= ZCL_COMM_FLAG_DELAYED_RESTART | ZCL_COMM_FLAG_FACTORY_RESET | ZCL_COMM_FLAG_INSTALL;
...
void zcl_comm_factory_reset(uint16_t ms_delay)
{
    ...
    zcl_comm_state.flags |= ZCL_COMM_FLAG_DELAYED_RESTART | ZCL_COMM_FLAG_FACTORY_RESET | ZCL_COMM_FLAG_INSTALL;
}

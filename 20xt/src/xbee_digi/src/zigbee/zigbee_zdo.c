...
*/
zigbee_zdo_debug const addr64* _zdo_envelope_create(wpan_envelope_t* envelope, wpan_dev_t* dev, const addr64* address)
{
    if (envelope == NULL) {
    ...
    {
        wpan_envelope_create(envelope, dev, &dev->address.ieee, dev->address.network);
...
*/
zigbee_zdo_debug int zdo_send_response(const wpan_envelope_t FAR* request, const void FAR* response, uint16_t length)
{
    int retval;
...
*/
zigbee_zdo_debug int zdo_match_desc_request(void* buffer, int16_t buflen, uint16_t addr_of_interest,
                                            uint16_t profile_id, const uint16_t* inClust, const uint16_t* outClust)
{
    XBEE_PACKED(, {
        uint8_t transaction;
...
zigbee_zdo_debug int _zdo_match_desc_respond(const wpan_envelope_t FAR* envelope)
{
    ...
    while ((ep = wpan_endpoint_get_next(envelope->dev, ep)) != NULL) {
        if (ep->endpoint != WPAN_ENDPOINT_ZDO && ep->profile_id == profile_id) {
            ...
            for (mask = WPAN_CLUST_FLAG_INPUT, type = 2; type && search; mask = WPAN_CLUST_FLAG_OUTPUT, --type) {
                count = *p.count++;
                while (search && count--) {
                    if (wpan_cluster_match(le16toh(*p.cluster_le++), mask, ep->cluster_table)) {
#ifdef ZIGBEE_ZDO_VERBOSE
...
zigbee_zdo_debug int zdo_simple_desc_request(wpan_envelope_t* envelope, uint16_t addr_of_interest,
                                             uint_fast8_t endpoint, wpan_response_fn callback, const void FAR* context)
{
    int retval;
    ...
    retval = wpan_conversation_register(zdo_endpoint_state(envelope->dev), callback, context, ZDO_CONVERSATION_TIMEOUT);
    if (retval < 0) {
        return retval;
    }
...
#define MAX_CLUSTERS ((sizeof rsp.clusters - 2) / sizeof(uint16_t))
...
#ifdef ZIGBEE_ZDO_VERBOSE
    printf("%s: request for endpoint 0x%02X:\n", __FUNCTION__, rsp.descriptor.endpoint);
#endif
    ...
    {
        ep = wpan_endpoint_match(envelope->dev, rsp.descriptor.endpoint, WPAN_APS_PROFILE_ANY);
    ...
    if (ep != NULL) {
        ...
        retval = _simple_desc_cluster_list(&buffer, MAX_CLUSTERS, WPAN_CLUST_FLAG_INPUT, ep->cluster_table);
        if (retval >= 0) {
            retval =
                _simple_desc_cluster_list(&buffer, MAX_CLUSTERS - retval, WPAN_CLUST_FLAG_OUTPUT, ep->cluster_table);
        }
        if (retval < 0) {
    ...
    return zdo_send_response(envelope, &rsp, offsetof(struct _response, descriptor));
...
zigbee_zdo_debug int _zdo_active_ep_respond(const wpan_envelope_t FAR* envelope)
{
    ...
    rsp.transaction            = ((const zdo_active_ep_req_t FAR*)envelope->payload)->transaction;
    rsp.header.status          = ZDO_STATUS_SUCCESS;
    rsp.header.network_addr_le = htole16(envelope->dev->address.network);
...
*/
zigbee_zdo_debug int zdo_handler(const wpan_envelope_t FAR* envelope, wpan_ep_state_t FAR* ep_state)
{
    uint8_t response[2];
    ...
    if (ZDO_CLUST_IS_RESPONSE(envelope->cluster_id) || (envelope->options & WPAN_ENVELOPE_BROADCAST_ADDR)) {
#ifdef ZIGBEE_ZDO_VERBOSE
        printf("%s: ignoring  %scast to unsupported cluster 0x%" PRIx16 "\n", __FUNCTION__,
               (envelope->options & WPAN_ENVELOPE_BROADCAST_ADDR) ? "broad" : "uni", envelope->cluster_id);
#endif
        return 0;
    }
...
/*** EndHeader */
// documented in zigbee/zdo.h
zigbee_zdo_debug int zdo_send_bind_req(wpan_envelope_t* envelope, uint16_t type, wpan_response_fn callback,
                                       void FAR* context)
{
    ...
    trans = wpan_conversation_register(zdo_endpoint_state(bind_env.dev), callback, context, ZDO_CONVERSATION_TIMEOUT);
    if (trans < 0) {
#ifdef ZIGBEE_ZDO_VERBOSE
...
zigbee_zdo_debug int zdo_mgmt_leave_req(wpan_dev_t* dev, const addr64* address, uint16_t flags)
{
    ...
    zdo.request.device_address = *_zdo_envelope_create(&envelope, dev, address);
...
/*** EndHeader */
zigbee_zdo_debug int zdo_send_descriptor_req(wpan_envelope_t* envelope, uint16_t cluster, uint16_t addr_of_interest,
...
// documented in zigbee/zdo.h
zigbee_zdo_debug int zdo_send_nwk_addr_req(wpan_dev_t* dev, const addr64 FAR* ieee_be, uint16_t FAR* net)
{
    ...
    transaction =
        wpan_conversation_register(zdo_endpoint_state(dev), _zdo_process_nwk_addr_resp, net, ZDO_CONVERSATION_TIMEOUT);
    if (transaction < 0) {
        return transaction;
    }
                                                 ...
                                                 const wpan_envelope_t FAR* envelope)
{
    ...
    if (envelope == NULL) {
        ...
        *ieee_be = *ZDO_IEEE_ADDR_TIMEOUT;
        ...
        response = envelope->payload;
        if (response->header.status == ZDO_STATUS_SUCCESS) {
            memcpy_letobe(ieee_be, &response->header.ieee_remote_le, sizeof *ieee_be);
#ifdef ZIGBEE_ZDO_VERBOSE
...
// documented in zigbee/zdo.h
zigbee_zdo_debug int zdo_send_ieee_addr_req(wpan_dev_t* dev, uint16_t net_addr, addr64 FAR* ieee_be)
{
    ...
    transaction = wpan_conversation_register(zdo_endpoint_state(dev), _zdo_process_ieee_addr_resp, ieee_be,
                                             ZDO_CONVERSATION_TIMEOUT);
    if (transaction < 0) {
        return transaction;
    }

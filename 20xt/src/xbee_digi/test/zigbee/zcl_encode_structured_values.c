...
const zcl_array_t tod_slots_array_attr       = {8, 8, 1, ZCL_TYPE_UNSIGNED_8BIT, tod_slots};
const zcl_array_t tod_slots_array_empty_attr = {8, 0, 1, ZCL_TYPE_UNSIGNED_8BIT, tod_slots};

const zcl_array_t big_nothing_array = {0xABCD, 0xABCD, 0, ZCL_TYPE_NO_DATA, ""};

const struct {
    uint8_t x;
    uint16_t y;
} big_slots[8] = {{1, 2}, {3, 4}, {5, 6}, {7, 8}};

const zcl_array_t big_slots_u8_attr  = {4, 4, sizeof(big_slots[0]), ZCL_TYPE_UNSIGNED_8BIT, &big_slots[0].x};
const zcl_array_t big_slots_u16_attr = {4, 4, sizeof(big_slots[0]), ZCL_TYPE_UNSIGNED_16BIT, &big_slots[0].y};
...
const test_struct_value_t test_struct_value = {
    0x1234, 0x55, TRUE, 0x0ABCDEF0, {4, 4, sizeof(big_slots[0]), ZCL_TYPE_UNSIGNED_8BIT, &big_slots[0].x}};

const zcl_struct_t test_struct_meta = {5, &test_struct_value, test_struct_elements};

const zcl_struct_t test_struct_with_array_meta = {5, &test_struct_value, test_struct_with_array};

const test_data_t test_data[] = {
    {{0x1234, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_ARRAY, &tod_slots_array_attr},
     11,
     {ZCL_TYPE_UNSIGNED_8BIT, 0x08, 0x00, 1, 2, 3, 4, 5, 6, 7, 8}},
    {{0x1234, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_ARRAY, &tod_slots_array_empty_attr},
     3,
     {ZCL_TYPE_UNSIGNED_8BIT, 0x00, 0x00}},
    {{0x1234, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_ARRAY, &big_slots_u8_attr},
     7,
     {ZCL_TYPE_UNSIGNED_8BIT, 0x04, 0x00, 1, 3, 5, 7}},
    {{0x1234, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_ARRAY, &big_slots_u16_attr},
     11,
     {ZCL_TYPE_UNSIGNED_16BIT, 0x04, 0x00, 2, 0, 4, 0, 6, 0, 8, 0}},
    {{0x1234, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_ARRAY, &big_nothing_array}, 3, {ZCL_TYPE_NO_DATA, 0xCD, 0xAB}},
    {{0x1234, ZCL_ATTRIB_FLAG_NONE, ZCL_TYPE_STRUCT, &test_struct_meta},
     15,
     {0x05, 0x00, ZCL_TYPE_UNSIGNED_8BIT, 0x55, ZCL_TYPE_UNSIGNED_16BIT, 0x34, 0x12, ZCL_TYPE_UNSIGNED_32BIT, 0xF0,
      0xDE, 0xBC, 0x0A, ZCL_TYPE_LOGICAL_BOOLEAN, TRUE, ZCL_TYPE_NO_DATA}},
...
void t_encode(void)
{
    ...
    for (i = 0; i < TEST_COUNT; ++i) {
        ...
        retval = zcl_encode_attribute_value(buffer, test_data[i].encoded_length - 1, &test_data[i].attr);
        ...
        memset(buffer, 0, sizeof buffer);
        retval = zcl_encode_attribute_value(buffer, test_data[i].encoded_length, &test_data[i].attr);
        ...
        if (!test_compare(retval, test_data[i].encoded_length, "%ld", errstr)) {
            ...
            match = 0 == memcmp(buffer, test_data[i].encoded, test_data[i].encoded_length);
    ...
    sprintf(errstr, "should have passed %lu tests", (unsigned long)TEST_COUNT * 3);
    test_info(errstr);
}

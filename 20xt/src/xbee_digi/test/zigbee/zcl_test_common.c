...
const zcl_attribute_tree_t attribute_tree[] = {{ZCL_MFG_NONE, attributes, NULL}};

const wpan_cluster_table_entry_t master_clusters[] = {
    {TEST_CLUSTER, &zcl_general_command, attribute_tree, WPAN_CLUST_FLAG_SERVER | WPAN_CLUST_FLAG_ENCRYPT},
...
const wpan_endpoint_table_entry_t endpoint_table[] = {ZDO_ENDPOINT(zdo_ep_state),
                                                      {
...
int test_send(const wpan_envelope_t FAR* envelope, uint16_t flags)
{
    ...
    ++response_count;
    passed =
        (envelope->length == expected_length && memcmp(envelope->payload, expected_response, expected_length) == 0);
...
void reset_common(const uint8_t* payload, int payload_length, const uint8_t* response, int response_length)
{
    dev.tick            = test_tick;

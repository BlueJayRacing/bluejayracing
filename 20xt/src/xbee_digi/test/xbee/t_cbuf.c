...
void cbuf_valid(xbee_cbuf_t* cbuf, unsigned free, unsigned used, unsigned linenum)
{
    char errmsg[80];
...
void test_get_and_put(void) // This test shows various get and put calls
{
    ...
    test_compare(xbee_cbuf_put(rxBuffer, source_data, 1), 0, NULL,
                 "put into full failed"); // test 6 (try to add another even though full, stores less than required)
    CBUF_VALID(rxBuffer, 0, 31);

    test_compare(xbee_cbuf_get(rxBuffer, read_buffer, 31), 31, NULL,
                 "get from full failed"); // test 7 (get and empty whole buffer)
    ...
    test_compare(xbee_cbuf_get(rxBuffer, read_buffer, 22), 22, NULL,
                 "get from full 5 failed"); // test 13 (get an arbitrary amount)
    ...
    test_compare(xbee_cbuf_put(rxBuffer, source_data, 31), 12, NULL,
                 "put from full 7 failed"); // test 19 (stores less than required)
    CBUF_VALID(rxBuffer, 0, 31);

    test_compare(xbee_cbuf_get(rxBuffer, read_buffer, 31), 31, NULL, "get from full 8 failed"); // test 22
    CBUF_VALID(rxBuffer, 31, 0);
    test_bool((memcmp(read_buffer, source_data + 22, 9) == 0) && (memcmp(read_buffer + 9, source_data, 10) == 0) &&
                  (memcmp(read_buffer + 19, source_data, 12) == 0),
              "Contents from the buffer are corrupted after 8");

    test_compare(xbee_cbuf_get(rxBuffer, read_buffer, 3), 0, NULL,
                 "get from full 9 failed"); // test 25 (gets less than required)
    CBUF_VALID(rxBuffer, 31, 0);
}
...
void test_tail_wrapping(void) // This test show that put correctly handles case of exactly filling the buffer
{
    ...
    CBUF_VALID(rxBuffer, 31, 0);
    test_bool((memcmp(read_buffer, source_data + 3, 12) == 0) && (memcmp(read_buffer + 12, source_data, 19) == 0),
...
void test_wrap_around(void) // This test show that put and get successfully wraps around
{
    ...
    test_compare(xbee_cbuf_get(rxBuffer, read_buffer, 31), 31, NULL,
                 "get from full 4 failed"); // test 12 test get wrap around
    CBUF_VALID(rxBuffer, 31, 0);
    test_bool((memcmp(read_buffer, source_data + 3, 12) == 0) && (memcmp(read_buffer + 12, source_data, 17) == 0) &&

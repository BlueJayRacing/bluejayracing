...
int test_footer(void)
{
    if (test_verbose) {
        if (_test_results.failures) {
            ...
            printf("%s: PASSED (%d tests)\n", _test_results.testname, _test_results.tests);
        }
    }
...
int test_compare(long result, long expected, const char* fmt, const char* error)
{
    char format_str[80];
    ...
    if (test_verbose) {
        ...
        sprintf(format_str, " failure #%d/test #%d: %%s (%s != %s)\n", _test_results.failures, _test_results.tests, fmt,
                fmt);
...
int test_string(const char* result, const char* expected, const char* error)
{
    ...
    if (test_verbose) {
        printf(" failure #%d/test #%d: %s ([%s] != [%s])\n", _test_results.failures, _test_results.tests, error,
               (result == NULL) ? "(null)" : result, (expected == NULL) ? "(null)" : expected);
    }

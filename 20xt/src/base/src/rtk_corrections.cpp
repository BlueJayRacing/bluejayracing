...
std::string read_raw_rtcm(serialib* serial)
{ // return the first RTCM message it finds
    uint8_t last = 0x00;
    uint8_t cur;

    // int     readBytes   (void *buffer, unsigned int maxNbBytes,const unsigned int timeOut_ms=0, unsigned int
    // sleepDuration_us=100);
    ...
    try {
        if (serial->readBytes((void*)(data + 3), length) !=
            length) { // TODO: Jonathan, can you check this line? I had to fix it
            throw -1;
        }
    } catch (int num) {
...
int main()
{
    ...
    mqd_t tx_queue = BajaIPC::open_queue(StationIPC::RTK_CORRECTOR_TO_PRIORITIZER, false);
    if (tx_queue == -1) {

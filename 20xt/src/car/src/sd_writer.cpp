...
int main() {
  ...
  string filenames = {"gps.csv", "localization.csv", "communication.csv", "analog_ch.csv", "car_state.csv",
                      "rtk_correction.csv"};
  ...
  for (int i = 0; i < NUM_FIELDS; i++) {
    f.emplace_back(ofstream(filenames[i]));
    write_line(f[i], headers[i]);
  }
  ...
  while (true) {
    ...
    cout << live_comm.DebugString() << endl;
  }
...
void log_communication(ofstream f, Observation observation) {
  ...
  if(com.has_instruction()){
    switch(com.instruction()){
      case STOP_IMMEDIATELY: instruction = "STOP_IMMEDIATELY";
      case STOP_FOR_PIT: instruction = "STOP_FOR_PIT";
      default: instruction = "";
    }
  }
  if(com.has_driver_response()){
    switch(com.driver_response()){
      case SEND_HELP: driver_response = "SEND_HELP";
      case PIT_REQUEST: driver_response = "PIT_REQUEST";
...
void log_analog_ch(ofstream f, Observation observation) {
  ...
  switch(ana.channel_type()){
    case AXLE_TORQUE_FRONT_LEFT: channel_type = "AXLE_TORQUE_FRONT_LEFT";
    case AXLE_TORQUE_FRONT_RIGHT: channel_type = "AXLE_TORQUE_FRONT_RIGHT";
    case AXLE_TORQUE_REAR_LEFT: channel_type = "AXLE_TORQUE_REAR_LEFT";
    case AXLE_TORQUE_REAR_RIGHT: channel_type = "AXLE_TORQUE_REAR_RIGHT";

    case AXLE_RPM_FRONT_RIGHT: channel_type = "AXLE_RPM_FRONT_RIGHT";
    case AXLE_RPM_FRONT_LEFT: channel_type = "AXLE_RPM_FRONT_LEFT";
    case AXLE_RPM_REAR: channel_type = "AXLE_RPM_REAR";

    case SHOCK_LEN_FRONT_RIGHT: channel_type = "SHOCK_LEN_FRONT_RIGHT";
    case SHOCK_LEN_FRONT_LEFT: channel_type = "SHOCK_LEN_FRONT_LEFT";
    case SHOCK_LEN_REAR_RIGHT: channel_type = "SHOCK_LEN_REAR_RIGHT";
    case SHOCK_LEN_REAR_LEFT: channel_type = "SHOCK_LEN_REAR_LEFT";

    case STEERING_DATA: channel_type = "STEERING_DATA";
    case TACHOMETER: channel_type = "TACHOMETER";
    case POWER_USE: channel_type = "POWER_USE";

    default: channel_type = "";
  }
  string encoded_analog_points = ana.encoded_analog_points();
...
void log_car_state(ofstream f, Observation observation) {
  ...
  if(cs.has_engine_status()){
    switch(cs.engine_status()){
      case ENGINE_NORMAL: engine_status = "ENGINE_NORMAL";
      case ENGINE_IDLING: engine_status = "ENGINE_IDLING";
      case ENGINE_CHOKED: engine_status = "ENGINE_CHOKED";
      case ENGINE_UNDERPERFORMING: engine_status = "ENGINE_UNDERPERFORMING";
      case ENGINE_OFF: engine_status = "ENGINE_OFF";
    }
  }

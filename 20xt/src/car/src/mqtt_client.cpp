...
int msgarrvd(void* context, char* topicName, int topicLen, MQTTClient_message* message)
{
    ...
    // printf("     topic: %s\n", topicName);
    uint32_t start_time = (*((uint8_t*)message->payload) << 24) + (*((uint8_t*)message->payload + 1) << 16) +
                          (*((uint8_t*)message->payload + 2) << 8) + (*((uint8_t*)message->payload + 3));
    ...
    for (int i = 0; i < 40; i++) {
        ...
        uint16_t val =
            *(((uint8_t*)message->payload) + 4 + 2 * i) * 256 + *(((uint8_t*)message->payload) + 4 + 2 * i + 1);

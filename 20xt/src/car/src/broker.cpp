...
int main()
{
    ...
    if (to_transmit_queue == -1) {
        std::cout << "Failed to get broker-to-transmit-prioritizer queue"
    ...
    if (sd_writer_queue == -1) {
        std::cout << "Failed to get broker-to-sd-writer queue"
                  << " Errno " << errno << std::endl;
        return EXIT_FAILURE;
    }
...
void try_broker_data(const std::vector<mqd_t>& producer_queues, const mqd_t to_transmit_queue,
                     const mqd_t sd_writer_queue)
{
    int i = 0;

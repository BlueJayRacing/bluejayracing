...
string hs_time(std::chrono::high_resolution_clock::time_point begin)
{
    ...
    return format("{}:{}:{}.{}{}", aTime->tm_hour, minutes.count(), seconds.count(), milliseconds.count(),
                  microseconds.count());
}
    ...
    while (1) {
        ...
        for (int i = 0; i < ADC_LOG_LENGTH; i++) {
            ...
            std::this_thread::sleep_until(iter_start +
                                          i * std::chrono::duration<double, std::milli>((1000.0 / ADC_RATE)));
        }
        temp.close();
        iter++;
    }
...
void imu_worker(string file_prefix, std::chrono::high_resolution_clock::time_point begin)
{
    ...
    while (1) {
        ...
        for (int i = 0; i < IMU_LOG_LENGTH; i++) {
            ...
            std::this_thread::sleep_until(iter_start +
                                          i * std::chrono::duration<double, std::milli>((1000.0 / IMU_RATE)));
        }
        temp.close();
        iter++;
    }

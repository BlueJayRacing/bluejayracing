...
int main(int argc, char* argv[])
{
    ...
    // histogram variables
    uint64_t shortHist[NHIST], longHist[NHIST], gtodHist[NHIST], emptyHist[NHIST], gpioHist[NHIST];
    for (int i = 0; i < NHIST; ++i)
        shortHist[i] = longHist[i] = gtodHist[i] = emptyHist[i] = gpioHist[i] = 0;
    ...
    for (int minutes = 1; minutes < 24 * 60; ++minutes) {
        ...
        while (Realtime::micros() - start < 60000000) {
            ...
            while (gpioRead(GPIO_IN) == 0 && Realtime::micros() - now < 1000)
                ;
            now   = Realtime::micros();
            ready = false;
            while (gpioRead(GPIO_IN) != 0 && Realtime::micros() - now < 1000)

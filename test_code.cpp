 ...
 *			  Credit to the following for the base no_os AD717X
 *library: acozma (andrei.cozma@analog.com) dnechita (dan.nechita@analog.com)
 *******************************************************************************/
#pragma once
#ifndef _AD717X_HPP_
#define _AD717X_HPP_
 ...
 **/
typedef enum ad717x_order { SINC5_SINC1 = 0x0, SINC3 = 0x3 } ad717x_order_t;
...
#define AD717X_IFMODE_REG_XOR_STAT(x)                                          \
  (((x) & AD717X_IFMODE_REG_XOR_EN) == AD717X_IFMODE_REG_XOR_EN)
#define AD717X_IFMODE_REG_CRC_STAT(x)                                          \
  (((x) & AD717X_IFMODE_REG_CRC_EN) == AD717X_IFMODE_REG_CRC_EN)
...
public:
  ...
  int32_t init(ad717x_init_param_t &t_init_param, SPIClass *t_spi_host,
               int8_t t_cs_pin);
  ...
  int32_t connectAnalogInput(uint8_t t_channel_id,
                             ad717x_analog_inputs_t t_analog_input);
  int32_t assignSetup(uint8_t t_channel_id, uint8_t t_setup_id);
  int32_t setPolarity(bool t_bipolar, uint8_t t_setup_id);
  int32_t setReferenceSource(ad717x_ref_source_t t_ref_source,
                             uint8_t t_setup_id);
  ...
  int32_t configureFilter(ad717x_filter_config_t t_filt_config,
                          uint8_t t_setup_id);

...
int clientsend(int cmd)
{
    ...
    if (sendstring(node, coms) != 0) { // read reply from connected node - not mesh
        clientread(node);
    }
...
int server()
{
    ...
    } else if (serverflag == 0 || serverflag == 1) { // node or classic
...
int le_callback(int clientnode, int operation, int cticn)
{
    ...
    if (operation == LE_CONNECT)
        ...
        for (int n = 0; n < 20; n++) {
            int startIndex            = count * 20;
            dataArray[startIndex + n] = (int)dat[n];
        }
        count += 1;
...
int node_classic_callback(int clientnode, char* buf, int nread)
{
    ...
    if (buf[0] == endchar || (nread == 2 && buf[0] != 0 && buf[1] == endchar) ||
        (nread == 3 && buf[0] != 0 && buf[1] <= 13 && buf[2] == endchar) || buf[0] == 'F' || buf[0] == 'X' ||
        buf[0] == 'Y' || buf[0] == 'G')
    ...
    if (nread != 0) {
        for (k = 0; k < nread; ++k) { // strip non-ascii chars for print
    ...
    if (firstc == endchar || firstc == 'p') {
    ...
    } else if (firstc == 'F') { // receive file
        // command = Ffilename  endchar stripped
        receivefile(&buf[1], clientnode);
    } else if (firstc == 'X' && nread > 1) { // destination directory for get file
        s = buf + 1;
        ...
        destdir[n] = 0;
    } else if (firstc == 'Y' && nread > 1) { // nblock for get file
        k    = 0;
        s    = buf + 1;
        flag = 0;
        n    = 0;
    ...
    } else if (firstc == 'G' && nread > 1) { // get file request with file name
        s = buf + 1;
        n = 0;
        while (s[n] != 0 && n < nread && n < 255) {
            temps[n] = s[n];
...
int clientconnect()
{
    ...
    if (device_type(node) == BTYPE_CL) { // classic server needs method = CHANNEL_STORED
                                         // or method = CHANNEL_NEW and channel
        if (inputchan(node, &channel, &method) < 0) {
            printf("Cancelled\n");
            return (0);
        }
    } else if (device_type(node) == BTYPE_ME) { // mesh device can be listening as Classic, LE or node server
        if (contype == 0)
            method = CHANNEL_NODE; // node server
        else if (contype == 1) {   // classic server
...
int sendfilex(int node, char* opcode, char* filename, char* destdir, int blocksize, int termchar)
{
    ...
    n  = 0;
    while (filename[n] > 32 && n < 1022) { // strip non alpha
        if (filename[n] == '/')
    ...
    if (destdir[0] != 0) {
        ...
        if (n != 0) {
            if (!(destdir[n - 1] == '/' || destdir[n - 1] == '\\')) { // backslash allowed for Classic/Windows
    ...
    if (getout != 0) { // error may have left ack in buffer
        read_node_clear(node);
        if (getout == 1)
            printf("Timed out\n");
        else
        ...
        if (gotn == 0)
            ...
            --gotn;
            while (gotn >= 0) { // strip trailing endchar and any cr/lf
...
int receivefilex(char* fname, int clientnode)
{
    ...
    do {
        ...
        if (nread == nblock) // got nblock chars
        {
            ...
            if (getout == 0) {
                ...
                if (ntogo != 0) { // ack send not last block
                    temps[0] = 10;
                    if (write_node(clientnode, temps, 1) != 1) // send one endchar ack byte
                        getout = 1;                            // error
                }
    ...
    if (getout != 0 || crc != 0) { // error may have left data in buffer

...
int init_blue_ex(char* filename, int hcin)
{
  ...
  while (readret > 0 && errcount == 0) {
    ...
    if (flag != 0) {
      ...
      if (flag == 1) {
        if (gpar.btleflag != 0 ||
            devnfrombadd(dev[dn]->baddr, BTYPE_XALL, DIRN_FOR) == 0) { // is device 0 board address local - move to dn=0
  ...
  do {
    ...
    if (buf[0] == 4) // HCI events
    {
      ...
      if (disflag != 0 && devicen != 0) {
        ...
        if (dp->conflag != 0 && gpar.btleflag == 0 && !((dp->conflag & CON_RF) == 0 && (dp->conflag & CON_SERVER) != 0))
        ...
        } else if (chan >= 0x40) // dynamic channel
        {
          ...
          if ((sflag & IN_SSAREP) != 0 && (buf[9] == 1 || buf[9] == 3 || buf[9] == 5 || buf[9] == 7) && buf[10] == 0)
            ...
            gotflag = IN_PNRSP;
                   ...
                   (buf[12] == 0xE1 || buf[12] == 0xE3 || buf[12] == 0x93 || buf[12] == 0x91)) { // copy for reply
        ...
        if (gotflag == IN_DATA) {
          ...
          } else // classic serial data first packet
          {
            ...
            if (datlen != 0) // length not zero    FF 01 credit ?
            {
              ...
              if (buf[5] + (buf[6] << 8) + 9 - wantlen != 0) { // multiple packets - this packet less than datlen
                datlen = wantlen - 12 - k;
              }
              if (datlen > 0)
                dev[devicen]->nx = pushins(gotflag, devicen, datlen, datp);
    ...
    if ((xflag == 0 && doneflag == 0 && mustflag != 0 && locmustflag == 0) ||
        (disflag != 0 && ndevice == devicen)) { // done - switch to short timeout
    ...
    do // until no more IN_DATA on stack
    {
      ...
      if (n >= 0) {                                       // data on instack - may be on first entry
        ...
        if (devicen == 0) {
           ...
           if ((devok(ndev) != 0) &&
               ((((*ndevice & FROM_MESH) != 0) && (dev[ndev]->type == BTYPE_ME)) ||
                ...
                (((*ndevice & FROM_MESHCON) != 0) && (dev[ndev]->conflag & CON_MESH) != 0))) { // read from this device
            devicen  = ndev;
        ...
        do // while still something on instack
        {
          ...
          if (meshflag == 0 &&
              (((flag & EXIT_CHAR) != 0 && lastchar == endchar) || ((flag & EXIT_COUNT) != 0 && gotn == count))) {
            popins();
            gpar.readerror = 0; // OK
            timems(TIM_FREE);

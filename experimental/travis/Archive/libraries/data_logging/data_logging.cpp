...
BajaDataWriter::BajaDataWriter(const string log_directory)
    : log_directory(log_directory), config_file_path(log_directory + "/" + CONFIG_FILE_NAME), current_data_file_num(-1),
...
int BajaDataWriter::add_channel(string channel_name, string unit_of_measurement,
                                ...
                                string time_unit, string num_bits_per_sample)
{
    std::string all_strings =
        channel_name + unit_of_measurement + scale_factor_decimal + time_unit + num_bits_per_sample;
    if (all_strings.find(',') != std::string::npos || all_strings.find(':') != std::string::npos ||
        all_strings.find('"') != std::string::npos || all_strings.find(' ') != std::string::npos ||
        all_strings.find('\n') != std::string::npos || all_strings.find('{') != std::string::npos ||
    ...
    ChannelDescription channel_description(channel_name, unit_of_measurement, scale_factor_decimal, time_unit,
                                           num_bits_per_sample, channel_id);
...
int BajaDataWriter::read_or_create_config()
{
    ...
    for (std::string json : jsons) {
        try {
            ...
            std::cerr << "Invalid json in config file, completely overwriting with empty config. Reason: " << e.what()
                      << std::endl;
...
int BajaDataWriter::open_new_data_file()
{
    ...
    string data_file_path = this->log_directory + "/" + DATA_FILE_PREFIX + std::to_string(this->current_data_file_num) +
                            DATA_FILE_EXTENSION;

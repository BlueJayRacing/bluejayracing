...
void setup() {
  ...
  setupConfig.setup.ref_source = EXTERNAL_REF2;
  // Choose an output data rate; for example, SPS_2500 (adjust as needed)
  setupConfig.filter_config.odr = SPS_50;
  setupConfig.gain = 1.0f;
...
if (currentMillis - lastPrintTime >= PRINT_INTERVAL_MS) {
    ...
    // Compute a per-channel rate (samples per second).
    double rate = 0.0;
    if (16 > 0) {
      rate = (static_cast<double>(totalCount)) * (1000.0 / PRINT_INTERVAL_MS);
    }
    ...
    // Print the data in aligned columns with two decimals
    // The width specifier (e.g. 10.2f) can be adjusted for more/less spacing.
    char buffer[32];

    // Print rate in the first column
    sprintf(buffer, "%10.2f", rate);
    Serial.print(buffer);

...
InertialSenseROS::InertialSenseROS() : Node("inertial_sense")
{
    ...
    // refLLA_set_current_srv_ = nh_.advertiseService("set_refLLA_current",
    // &InertialSenseROS::set_current_position_as_refLLA, this); refLLA_set_value_srv_ =
    // nh_.advertiseService("set_refLLA_value", &InertialSenseROS::set_refLLA_to_value, this); mag_cal_srv_ =
    // nh_.advertiseService("single_axis_mag_cal", &InertialSenseROS::perform_mag_cal_srv_callback, this);
    // multi_mag_cal_srv_ = nh_.advertiseService("multi_axis_mag_cal",
    // &InertialSenseROS::perform_multi_mag_cal_srv_callback, this); firmware_update_srv_ =
    // node->create_service<inertial_sense_ros::srv::FirmwareUpdate>("firmware_update",
    // &InertialSenseROS::update_firmware_srv_callback);
...
void InertialSenseROS::set_navigation_dt_ms()
{
    ...
    //     IS_.SendData(DID_FLASH_CONFIG, (uint8_t*)(&data), sizeof(uint32_t), offsetof(nvm_flash_cfg_t,
    //     startupNavDtMs)); ROS_INFO("navigation rate change from %dms to %dms, resetting uINS to make change",
    //     IS_.GetFlashConfig().startupNavDtMs, nav_dt_ms); sleep(3); reset_device();
    //   }
    // }
}
...
template <typename T> void InertialSenseROS::set_flash_config(std::string param_name, uint32_t offset, T def)
{
    T tmp;
...
// bool InertialSenseROS::update_firmware_srv_callback(std::shared_ptr<inertial_sense_ros::srv::FirmwareUpdate::Request>
// req, std::shared_ptr<inertial_sense_ros::srv::FirmwareUpdate::Request> res)
// {
//   IS_.Close();

...
InertialSenseROS::InertialSenseROS(bool configFlashParameters) : Node("inertial_sense"), initialized_(false), rtk_connectivity_watchdog_timer_()
{
  // Load parameters
  ...
  // refLLA_set_current_srv_ = this->create_service("set_refLLA_current", &InertialSenseROS::set_current_position_as_refLLA, this);
  // refLLA_set_value_srv_ = this->create_service("set_refLLA_value", &InertialSenseROS::set_refLLA_to_value, this);
  // mag_cal_srv_ = this->create_service("single_axis_mag_cal", &InertialSenseROS::perform_mag_cal_srv_callback, this);
  // multi_mag_cal_srv_ = this->create_service("multi_axis_mag_cal", &InertialSenseROS::perform_multi_mag_cal_srv_callback, this);
  // //firmware_update_srv_ = this->create_service("firmware_update", &InertialSenseROS::update_firmware_srv_callback, this);
  // data_stream_timer_ = this->create_wall_timer(rclcpp::Duration(1), configure_data_streams, this); // 2 Hz
  // if (diagnostics_.enabled)
  // {
  //     diagnostics_.pub = this->create_publisher<diagnostic_msgs::msg::DiagnosticArray>("diagnostics", 1);
  //     diagnostics_timer_ = this->create_wall_timer(rclcpp::Duration(0.5), &InertialSenseROS::diagnostics_callback, this); // 2 Hz
  // }
...
void InertialSenseROS::configure_data_streams(bool startup) // if startup is true each step will be attempted without returning
{
    ...
    if (GPS1_.enabled)
    {
        ...
        if (GPS1_raw_.enabled && !gps1RawStreaming_)
        {
            ...
            GPS1_raw_.pub = this->create_publisher<inertial_sense_ros_humble_msgs::msg::GNSSObsVec>(gps1_topic_ + "/obs", 50);
            GPS1_raw_.pub2 = this->create_publisher<inertial_sense_ros_humble_msgs::msg::GNSSEphemeris>(gps1_topic_ + "/eph", 50);
            GPS1_raw_.pub3 = this->create_publisher<inertial_sense_ros_humble_msgs::msg::GlonassEphemeris>(gps1_topic_ + "/geph", 50);
            SET_CALLBACK(DID_GPS1_RAW, gps_raw_t, GPS_raw_callback, gps_raw_period_multiple);
            GPS_base_raw_.pub = this->create_publisher<inertial_sense_ros_humble_msgs::msg::GlonassEphemeris>("/base_geph", 50);
            GPS_base_raw_.pub2 = this->create_publisher<inertial_sense_ros_humble_msgs::msg::GNSSEphemeris>(gps1_topic_ + "/base_eph", 50);
            GPS_base_raw_.pub3 = this->create_publisher<inertial_sense_ros_humble_msgs::msg::GlonassEphemeris>(gps1_topic_ + "/base_geph", 50);
    ...
    if (GPS2_.enabled)
    {
        ...
        if (GPS2_raw_.enabled && !gps2RawStreaming_)
        {
            ...
            GPS2_raw_.pub = this->create_publisher<inertial_sense_ros_humble_msgs::msg::GNSSObsVec>(gps2_topic_ + "/obs", 50);
            GPS2_raw_.pub2 = this->create_publisher<inertial_sense_ros_humble_msgs::msg::GNSSEphemeris>(gps2_topic_ + "/eph", 50);
            GPS2_raw_.pub3 = this->create_publisher<inertial_sense_ros_humble_msgs::msg::GlonassEphemeris>(gps2_topic_ + "/geph", 50);
...
//   RCLCPP_INFO(this->get_logger(), "Connected to uINS (SN: %d, HW Ver: %d.%d, FW Ver: %d.%d.%d.%d, Port: %s, Baud: %d)",
//               dev_info.serialNumber,
//               (dev_info.hardwareVer >> 8) & 0xFF, dev_info.hardwareVer & 0xFF,
//               (dev_info.firmwareVer >> 24) & 0xFF, (dev_info.firmwareVer >> 16) & 0xFF, (dev_info.firmwareVer >> 8) & 0xFF, dev_info.firmwareVer & 0xFF,
...
//                  (EXPECTED_FIRMWARE_VERSION >> 24) & 0xFF, (EXPECTED_FIRMWARE_VERSION >> 16) & 0xFF, (EXPECTED_FIRMWARE_VERSION >> 8) & 0xFF, EXPECTED_FIRMWARE_VERSION & 0xFF,
//                  (dev_info.firmwareVer >> 24) & 0xFF, (dev_info.firmwareVer >> 16) & 0xFF, (dev_info.firmwareVer >> 8) & 0xFF, dev_info.firmwareVer & 0xFF);
//     return false;
//   }
...
void InertialSenseROS::configure_flash_parameters()
{
    ...
    {
        reboot = true;
        RCLCPP_INFO(this->get_logger(), "navigation rate change from %dms to %dms, resetting uINS to make change", current_flash_cfg.startupNavDtMs, navigation_dt_ms_);
    }
    if (current_flash_cfg.ioConfig != ioConfig_)
    {
        RCLCPP_INFO(this->get_logger(), "ioConfig change from %x to %x, resetting uINS to make change", current_flash_cfg.ioConfig, ioConfig_);
        reboot = true;
    }
...
void InertialSenseROS::connect_rtk_client(const std::string &RTK_correction_protocol, const std::string &RTK_server_IP, const int RTK_server_port)
{
    ...
    while (RTK_connection_attempt_count < RTK_connection_attempt_limit_)
    {
        ...
        else
        {
            ...
            {
                RCLCPP_ERROR_STREAM(this->get_logger(), "Giving up after " << RTK_connection_attempt_count << " failed attempts");
            }
            else
            {
...
void InertialSenseROS::start_rtk_connectivity_watchdog_timer()
{
    ...
    rtk_connectivity_watchdog_timer_ = this->create_wall_timer(nsec, std::bind(&InertialSenseROS::rtk_connectivity_watchdog_timer_callback, this));
}
...
void InertialSenseROS::configure_rtk()
{
    ...
    else
    {
        ...
        RCLCPP_ERROR_EXPRESSION(this->get_logger(), RTK_rover_ && (RTK_base_serial_ || RTK_base_USB_ || RTK_base_TCP_), "unable to configure onboard receiver to be both RTK rover and base - default to rover");
        RCLCPP_ERROR_EXPRESSION(this->get_logger(), RTK_rover_ && GNSS_Compass_, "unable to configure onboard receiver to be both RTK rover as dual GNSS - default to dual GNSS");
    ...
    /*if (odom_ins_ned_.enabled || odom_ins_enu_.enabled || odom_ins_ecef_.enabled)
    {
        ...
        if (odom_ins_ecef_.enabled)
        {
            ...
            if (publishTf_)
            {
                ...
                transform_ECEF.setOrigin(tf2::Vector3(ecef_odom_msg.pose.pose.position.x, ecef_odom_msg.pose.pose.position.y, ecef_odom_msg.pose.pose.position.z));
                tf2::Quaternion q;
                tf2::quaternionMsgToTF(ecef_odom_msg.pose.pose.orientation, q);
                transform_ECEF.setRotation(q);

                br->sendTransform(tf2::StampedTransform(transform_ECEF, get_clock()->now(), "ins_ecef", "ins_base_link_ecef"));
            }
        }
        ...
        if (odom_ins_ned_.enabled)
        {
            ...
            {
                // Calculate the TF from the pose...
                transform_NED.setOrigin(tf2::Vector3(ned_odom_msg.pose.pose.position.x, ned_odom_msg.pose.pose.position.y, ned_odom_msg.pose.pose.position.z));
                tf2::Quaternion q;
                tf2::quaternionMsgToTF(ned_odom_msg.pose.pose.orientation, q);
                transform_NED.setRotation(q);

                br->sendTransform(tf2::StampedTransform(transform_NED, get_clock()->now(), "ins_ned", "ins_base_link_ned"));
            }
        }
        ...
        if (odom_ins_enu_.enabled)
        {
            ...
            {
                // Calculate the TF from the pose...
                transform_ENU.setOrigin(tf2::Vector3(enu_odom_msg.pose.pose.position.x, enu_odom_msg.pose.pose.position.y, enu_odom_msg.pose.pose.position.z));
                tf2::Quaternion q;
                tf2::quaternionMsgToTF(enu_odom_msg.pose.pose.orientation, q);
                transform_ENU.setRotation(q);

                br->sendTransform(tf2::StampedTransform(transform_ENU, get_clock()->now(), "ins_enu", "ins_base_link_enu"));
            }
        }
    }*/
}
...
void InertialSenseROS::INL2_states_callback(eDataIDs DID, const inl2_states_t *const msg)
{
    if (!inl2StatesStreaming_)
    {
        ...
        if (INL2_states_.enabled)
            INL2_states_.pub = this->create_publisher<inertial_sense_ros_humble_msgs::msg::INL2States>("inl2_states", 1);
    }
    ...
    if (DID = DID_GPS1_SAT)
    {
        if (!gps1InfoStreaming_)
        {
            ...
            if (GPS1_info_.enabled)
                GPS1_info_.pub = this->create_publisher<inertial_sense_ros_humble_msgs::msg::GPSInfo>(gps1_topic_ + "/info", 1);
    ...
    if (DID = DID_GPS2_SAT)
    {
        if (!gps2InfoStreaming_)
        {
            ...
            if (GPS2_info_.enabled)
                GPS2_info_.pub = this->create_publisher<inertial_sense_ros_humble_msgs::msg::GPSInfo>(gps1_topic_ + "/info", 1);
...
void InertialSenseROS::RTK_Rel_callback(eDataIDs DID, const gps_rtk_rel_t *const msg)
{
    ...
    if (abs(GPS_towOffset_) > 0.001)
    {
        ...
        {
            rtk_rel.e_gps_nav_fix_status = inertial_sense_ros_humble_msgs::msg::RTKRel::GPS_STATUS_FLAGS_RTK_FIX_AND_HOLD;
        }
...
bool InertialSenseROS::set_refLLA_to_value(inertial_sense_ros_humble_msgs::srv::Refllaupdate::Request &req, inertial_sense_ros_humble_msgs::srv::Refllaupdate::Response &res)
{
...
bool InertialSenseROS::update_firmware_srv_callback(inertial_sense_ros_humble_msgs::srv::Firmwareupdate::Request &req, inertial_sense_ros_humble_msgs::srv::Firmwareupdate::Response &res)
{
    //   IS_.Close();

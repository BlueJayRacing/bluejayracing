...
// Updated key: sourceId is now a string.
struct ChannelKey {
    std::string sourceId;
    uint32_t channelId;
...
struct AggregationConfig {
    uint64_t aggregationWindowMicroseconds = 10000000;  // Default 10 seconds.
    size_t maxMessages = 20000;                         // Maximum messages held.
    size_t downSamplingFactor = 10;                     // Down sampling factor.
};
...
public:
    ...
    // Now using a string for the sourceId.

...
void setup()
{
    ...
    rclc_publisher_init_default(&throttle_pub, &motor_node, ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32),
                                "throttle_cmd");
    rclc_publisher_init_default(&brake_pub, &motor_node, ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32),
                                "brake_cmd");

    // Create steering command node
    rclc_node_init_default(&steering_node, "steering_node", "", &support);
    rclc_publisher_init_default(&steering_pub, &steering_node, ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Float32),
                                "steering_cmd");

    // Create serial node
    rclc_node_init_default(&serial_node, "serial_node", "", &support);
    rclc_subscription_init_default(&cmd_sub, &serial_node, ROSIDL_GET_MSG_TYPE_SUPPORT(geometry_msgs, msg, Twist),
                                   "cmd_vel", motor_callback);
}

...
AxleTorqueAggregator::AxleTorqueAggregator(const std::string& name)
    : Node(name), message_counters_{0, 0, 0, 0} {
    ...
    subscription_ = this->create_subscription<baja_msgs::msg::Observation>(
        ...
        std::bind(&AxleTorqueAggregator::observation_callback, this, std::placeholders::_1),
        options);
...
void AxleTorqueAggregator::update_and_print_averages(const baja_msgs::msg::AnalogChannel& channel) {
    ...
    bool should_print = buffer.size() == MAX_BUFFER_SIZE &&
                        (counter % MAX_BUFFER_SIZE == 0 || 
                         now - last_print_time >= PRINT_INTERVAL);
    ...
    if (should_print) {
        ...
        RCLCPP_INFO(this->get_logger(), "Average Axle Torque (%s): %.2f, Total Messages: %zu", 
                    axle_names[index], average, counter);
        last_print_time = now;
    }
}

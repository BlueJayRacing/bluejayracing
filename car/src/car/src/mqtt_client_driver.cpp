...
static const char* HEX_ARRAY = "000102030405060708090A0B0C0D0E0F"
...
constexpr std::array<uint8_t, 256> HEX_VALUES = []() {
    ...
    for (int i = 0; i < 256; i++) {
        if (i >= '0' && i <= '9')
            ...
            arr[i] = 0xFF; // Invalid hex character
    }
    return arr;
}();
...
MQTTClientDriver::MQTTClientDriver() : Node("mqtt_client_driver")
{
    ...
    publisher_                 = create_publisher<baja_msgs::msg::Observation>(
...
int MQTTClientDriver::message_arrived(void* context, char* topicName, int topicLen, MQTTClient_message* message)
{
    ...
    if (mac_str == "84:FC:E6:00:99:98") {
        ...
        RCLCPP_ERROR(driver->get_logger(), "Unknown MAC address: %s, payload of len %i", mac_str.c_str(),
                     message->payloadlen);
    ...
    for (int i = 0; i < MQTT_MESSAGE_LENGTH; i++) {
        if (mqtt_msg.data[i].data_diff_micro == 0 && mqtt_msg.data[i].data_point == 0)
            break;
    ...
    // Calculate durations
    // auto initial_processing_duration = std::chrono::duration_cast<std::chrono::microseconds>(end_initial_processing -
    // start_initial_processing).count(); auto publishing_loop_duration =
    // std::chrono::duration_cast<std::chrono::microseconds>(end_publishing_loop - start_publishing_loop).count(); auto
    // total_duration = std::chrono::duration_cast<std::chrono::microseconds>(end_total - start_total).count();

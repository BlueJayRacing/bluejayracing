...
struct {
    ...
    uint8_t end_of_list;
} sample_endpoints = {ZDO_ENDPOINT(zdo_ep_state), WPAN_ENDPOINT_END_OF_LIST};
    ...
    while (1) {
        ...
        if (!strcmpi(cmdstr, "help") || !strcmp(cmdstr, "?")) {
            ...
            if (addr64_parse(&ieee_be, &cmdstr[5])) {
                ...
                err = zdo_send_nwk_addr_req(&my_xbee.wpan_dev, &ieee_be, &return_net_addr);
            ...
            if (tail == NULL) {
                ...
                err = zdo_send_ieee_addr_req(&my_xbee.wpan_dev, net_addr, &return_ieee_be);

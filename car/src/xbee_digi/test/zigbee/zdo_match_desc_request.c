...
const zcl_attribute_base_t master_attributes[] = {{ZCL_ATTRIBUTE_END_OF_LIST}};
                     ...
                     const char* error)
{
    ...
    passed = actual_length == expected_length && (memcmp(actual, expected, expected_length) == 0);
...
const uint16_t empty_clusters[] = {WPAN_CLUSTER_END_OF_LIST};

const uint16_t some_clusters[] = {TEST_CLUSTER, TEST_CLUSTER + 1, TEST_CLUSTER + 2, TEST_CLUSTER + 3,
                                  WPAN_CLUSTER_END_OF_LIST};

const uint16_t other_clusters[] = {TEST_CLUSTER + 4, TEST_CLUSTER + 5, TEST_CLUSTER + 6, WPAN_CLUSTER_END_OF_LIST};

uint8_t buffer[128];

void _t_common(const void* in, const void* out, const uint8_t* expected, size_t expected_size)
{
    int i, retval;

    for (i = 0; i < 2; ++i) {
        memset(buffer, 0xFF, sizeof buffer);
        retval = zdo_match_desc_request(buffer, expected_size - i, TEST_ADDR, TEST_PROFILE, in, out);

        if (i) {
            if (test_compare(retval, -ENOSPC, "%ld", "supposedly fit in small buffer")) {
                if (retval > 0) {
            ...
            compare_buffers(buffer, retval, expected, expected_size, "unexpected response");
...
void t_no_clusters(void)
{
    ...
    retval = zdo_match_desc_request(buffer, sizeof no_clusters_expected, TEST_ADDR, TEST_PROFILE, empty_clusters,
                                    empty_clusters);

    test_compare(retval, -ENOSPC, NULL, "unexpected response when asked to match no clusters");
}
...
void t_input_only(void) { _t_common(some_clusters, empty_clusters, input_only_expected, sizeof input_only_expected); }
...
{
    _t_common(empty_clusters, some_clusters, output_only_expected, sizeof output_only_expected);
}
...
void t_both(void) { _t_common(some_clusters, other_clusters, both_expected, sizeof both_expected); }

void t_null_buffer(void)
{
    int retval;

    retval = zdo_match_desc_request(NULL, sizeof both_expected, TEST_ADDR, TEST_PROFILE, some_clusters, other_clusters);

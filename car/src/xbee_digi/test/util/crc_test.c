...
int main(void)
{
    ...
    if (crc16buypass_bit(init, "\xda", 1) != crc16buypass_rem(crc16buypass_rem(init, 0xda, 3), 0xd0, 5))
        fputs("small bits mismatch for crc16buypass\n", stderr);
    if (crc16buypass_byte(0, NULL, 0) != init || crc16buypass_byte(init, "123456789", 9) != 0xfee8 ||
        crc16buypass_byte(init, data + 1, sizeof(data) - 1) != crc)
        fputs("byte-wise mismatch for crc16buypass\n", stderr);
    if (crc16buypass_word(0, NULL, 0) != init || crc16buypass_word(init, "123456789", 9) != 0xfee8 ||

...
#define jsll_print_init(n) printf("JSLL_INIT( 0x%08" PRIx32 ", 0x%08" PRIx32 " )", JSLL_HI(n), JSLL_LO(n))

void jsll_rand(uint64_t* n)
{
    // rand only generates 31 bits, so use 3 to get a random 64-bit value
    *n = ((uint64_t)(uint32_t)rand() << 33) ^ ((uint64_t)(uint32_t)rand() << 16) + (uint32_t)rand();
}
...
void jsll_gen(void)
{
    ...
    for (i = 0; i < 20; ++i) {
        ...
        i32  = (int32_t)a;
        ui32 = (uint32_t)a;
        printf("%" PRId32 ", UINT32_C(%" PRIu32 "),\t// (int32_t) a, (uint32_t) a\n\t", i32, ui32);

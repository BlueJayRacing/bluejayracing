...
uint8_t SecureVerifier[] = {
    ,
};
...
void t_srp(void)
{
    ...
    usr = srp_user_new(TEST_USERNAME, (const uint8_t*)TEST_BYTES_PW, TEST_LEN_PW);
    ...
    ver = srp_verifier_new(TEST_USERNAME, SecureSalt, sizeof(SecureSalt), SecureVerifier, sizeof(SecureVerifier),
    ...
    srp_user_process_challenge(usr, SecureSalt, sizeof(SecureSalt), bytes_B, len_B, &bytes_M, &len_M);
    ...
    if (test_bool(srp_verifier_is_authenticated(ver), "ver: authentication failed")) {
        goto test_abort;
    }
    srp_user_verify_session(usr, bytes_HAMK);

    if (test_bool(srp_user_is_authenticated(usr), "usr: authentication failed")) {
        goto test_abort;
    }
    ...
    if (usr_keylen == ver_keylen) {
        test_compare(memcmp(usr_key, ver_key, usr_keylen), 0, NULL, "keys do not match");
    }

...
/*** EndHeader */
const micro_info_t platform_avr[] = {{1, "AVR ATMega 64"}, {2, "AVR ATMega 128"}, {3, "AVR ATMega 32"}, MICRO_INFO_END};

const micro_info_t platform_xap2b[] = {{1, "EM250"}, {2, "EM260"}, MICRO_INFO_END};

const micro_info_t platform_cortex_m3[] = {{1, "EM350"}, {2, "EM360"}, {3, "EM357"},     {4, "EM367"},
                                           {5, "EM351"}, {6, "EM35x"}, {7, "STM32W108"}, MICRO_INFO_END};

const platform_info_t platform_info[] = {
    {1, platform_avr}, {2, platform_xap2b}, {4, platform_cortex_m3}, PLATFORM_INFO_END};
...
void ebl_header_dump_em3xx(const em3xx_header_t* ebl, uint16_t flags)
{
    ...
    if (flags & EBL_HEADER_DUMP_EMBER_VER) {
        printf("Ember Stack: %X.%X build 0x%02X  ", ebl->znet_release >> 4, ebl->znet_release & 0x0F, ebl->znet_build);
    }
    if (flags & EBL_HEADER_DUMP_TARGET_DESC) {
        printf("Target: %s  ", ebl_target_desc(ebl->plat_info, ebl->micro_info));
    }
    if (flags & EBL_HEADER_DUMP_PHY_DESC) {
        printf("Phy: %s  ", ebl_phy_desc(ebl->phy_info));
    }
    if (flags & (EBL_HEADER_DUMP_EMBER_VER | EBL_HEADER_DUMP_TARGET_DESC | EBL_HEADER_DUMP_PHY_DESC)) {
        puts("");
    }

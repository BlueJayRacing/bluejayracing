...
int xbee_ipv4_envelope_send(const xbee_ipv4_envelope_t FAR* envelope)
{
    ...
    xbee_ipv4_protocol_str(protocol_string, envelope->protocol)
        printf("%s: %u bytes to %s "
               ...
               __FUNCTION__, envelope->length, xbee_ipv4_ntoa(buf_addr, envelope->remote_addr_be),
               envelope->remote_port, envelope->local_port, protocol_string, envelope->options);
#endif

    retval = xbee_frame_write(xbee, &header, sizeof(header), envelope->payload, envelope->length, 0);
...
void xbee_ipv4_envelope_dump(const xbee_ipv4_envelope_t FAR* envelope, bool_t include_payload)
{
    ...
    printf("Message from %s:%u to %s port %u, %u bytes\n", buf_addr, envelope->remote_port,
           xbee_ipv4_protocol_str(buf_proto, envelope->protocol), envelope->local_port, envelope->length);
    if (include_payload) {

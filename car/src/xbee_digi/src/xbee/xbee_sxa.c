...
_xbee_sxa_debug sxa_node_t FAR*(sxa_list_head)(void) { return sxa_table; }

_xbee_sxa_debug int(sxa_list_count)(void) { return sxa_table_count; }

_xbee_sxa_debug xbee_dev_t*(sxa_xbee)(sxa_node_t FAR* sxa) { return sxa->xbee; }

_xbee_sxa_debug wpan_address_t FAR*(sxa_wpan_address)(sxa_node_t FAR* sxa) { return sxa->addr_ptr; }

/*** BeginHeader _sxa_default_cache_groups */
/*** EndHeader */

const FAR xbee_atcmd_reg_t _sxa_ni_query_regs[] = {XBEE_ATCMD_REG('N', 'I', XBEE_CLT_COPY, sxa_node_t, id.node_info),
                                                   XBEE_ATCMD_REG_END};

const FAR xbee_atcmd_reg_t _sxa_devinfo_query_regs[] = {
    XBEE_ATCMD_REG_MOVE_THEN_CB('A', 'O', XBEE_CLT_COPY_BE, sxa_node_t, ao, _sxa_process_version_info, 0),
    XBEE_ATCMD_REG('H', 'V', XBEE_CLT_COPY_BE, sxa_node_t, hardware_version),
    XBEE_ATCMD_REG_MOVE_THEN_CB('V', 'R', XBEE_CLT_COPY_BE, sxa_node_t, firmware_version, _sxa_process_version_info, 1),
    XBEE_ATCMD_REG('D', 'D', XBEE_CLT_COPY_BE, sxa_node_t, dd), XBEE_ATCMD_REG_END};

const FAR xbee_atcmd_reg_t _sxa_dhdl_query_regs[] = {
    XBEE_ATCMD_REG('D', 'H', XBEE_CLT_COPY, sxa_node_t, dest_addr.l[0]),
    XBEE_ATCMD_REG('D', 'L', XBEE_CLT_COPY, sxa_node_t, dest_addr.l[1]), XBEE_ATCMD_REG_END};
...
const FAR sxa_cached_group_t _sxa_default_cache_groups[] = {
    ...
    _SXA_CG_INIT(SXA_CACHED_DHDL, dhdl_cf, _sxa_dhdl_query_regs, NULL), _SXA_CG_INIT_END};
...
/*** EndHeader */
void _sxa_process_version_info(const xbee_cmd_response_t FAR* response, const struct xbee_atcmd_reg_t FAR* reg,
                               void FAR* base)
{
    ...
    */
    ...
    case XBEE_HARDWARE_S2C:
        ...
        sxa->caps |= ZB_CAP_ADV_ADDR | ZB_CAP_GW_FW | ZB_CAP_ZIGBEE | ZB_CAP_CHILDREN | ZB_CAP_ZDO | ZB_CAP_ZBPRO;
        switch (zbV) {
...
*/
_xbee_sxa_debug int _sxa_disc_process_node_data(xbee_dev_t* xbee, const void FAR* raw, int length)
{
...
*/
_xbee_sxa_debug int _sxa_disc_atnd_response(xbee_dev_t* xbee, const void FAR* raw, uint16_t length, void FAR* context)
{
    static const xbee_at_cmd_t nd              = {{'N', 'D'}};
    const xbee_frame_local_at_resp_t FAR* resp = raw;

    if (resp->header.command.w == nd.w && XBEE_AT_RESP_STATUS(resp->header.status) == XBEE_AT_RESP_SUCCESS) {
        // this is a successful ATND response
#ifdef XBEE_DISCOVERY_VERBOSE
        printf("%s\n", __FUNCTION__);
#endif
        return _sxa_disc_process_node_data(xbee, resp->value, length - offsetof(xbee_frame_local_at_resp_t, value));
    }
...
_xbee_sxa_debug int _sxa_disc_handle_frame_0x95(xbee_dev_t* xbee, const void FAR* raw, uint16_t length,
                                                void FAR* context)
{
    ...
    printf("%s\n", __FUNCTION__);
#endif
    return _sxa_disc_process_node_data(xbee, &frame->node_data, length - offsetof(xbee_frame_node_id_t, node_data));
}

/*** BeginHeader sxa_node_by_addr */
/*** EndHeader */
// search the node table for a node by its IEEE address
_xbee_sxa_debug sxa_node_t FAR* sxa_node_by_addr(const addr64 FAR* ieee_be)
{
    sxa_node_t FAR* rec;
...
_xbee_sxa_debug sxa_node_t FAR* sxa_local_node(const xbee_dev_t* xbee)
{
    sxa_node_t FAR* rec;
...
_xbee_sxa_debug sxa_node_t FAR* sxa_node_by_name(const char FAR* name)
{
    sxa_node_t FAR* rec;
...
_xbee_sxa_debug sxa_node_t FAR* sxa_node_by_index(int index)
{
    sxa_node_t FAR* rec;
...
_xbee_sxa_debug sxa_node_t FAR* sxa_node_add(xbee_dev_t* xbee, const xbee_node_id_t FAR* node_id)
{
    sxa_node_t FAR* rec;
    bool_t is_local;
    ...
    if (rec == NULL) {
        ...
        is_local = !memcmp(&rec->address.ieee, &xbee->wpan_dev.address.ieee, sizeof(rec->address.ieee));
        if (is_local) {
...
/*** EndHeader */
_xbee_sxa_debug void sxa_node_table_dump(void)
{
    sxa_node_t FAR* rec;
    ...
    for (rec = sxa_list_head(); rec; rec = rec->next) {
        ...
        printf("Addr:%08" PRIx32 "-%08" PRIx32 " 0x%04x  "
               "par:0x%04x %6s\n",
               be32toh(rec->id.ieee_addr_be.l[0]), be32toh(rec->id.ieee_addr_be.l[1]), rec->id.network_addr,
               rec->id.parent_addr, xbee_disc_device_type_str(rec->id.device_type));
        if (rec->device_info_cf == _SXA_CACHED_OK) {
            printf("\tHV:%04X VR:%08" PRIx32 " DD:%08" PRIx32 "\n", rec->hardware_version, rec->firmware_version,
                   rec->dd);
            printf("\tCAPS:%s%s%s%s%s%s%s%s%s%s\n", rec->caps & ZB_CAP_ADV_ADDR ? " ADV_ADDR" : "",
                   rec->caps & ZB_CAP_ZDO ? " ZDO" : "", rec->caps & ZB_CAP_REMOTE_DDO ? " REMOTE_DDO" : "",
                   rec->caps & ZB_CAP_GW_FW ? " GW_FW" : "", rec->caps & ZB_CAP_REMOTE_FW ? " REMOTE_FW" : "",
                   rec->caps & ZB_CAP_ZIGBEE ? " ZIGBEE" : "", rec->caps & ZB_CAP_ZBPRO ? " ZBPRO" : "",
                   rec->caps & ZB_CAP_DIGIMESH ? " DIGIMESH" : "", rec->caps & ZB_CAP_CHILDREN ? " CHILDREN" : "",
...
*/
_xbee_sxa_debug int _sxa_disc_cluster_handler(const wpan_envelope_t FAR* envelope, void FAR* context)
{
    ...
    printf("%s\n", __FUNCTION__);
#endif
    return _sxa_disc_process_node_data((xbee_dev_t*)envelope->dev, envelope->payload, envelope->length);
}
...
*/
_xbee_sxa_debug int _sxa_io_process_response(const addr64 FAR* ieee_be, const void FAR* raw, int length)
{
...
_xbee_sxa_debug int _sxa_io_response_handler(xbee_dev_t* xbee, const void FAR* raw, uint16_t length, void FAR* context)
{
    ...
    _sxa_io_process_response(&frame->ieee_address_be, &frame->num_samples, iolen);
...
/*** EndHeader */
_xbee_sxa_debug int _sxa_io_cluster_handler(const wpan_envelope_t FAR* envelope, void FAR* context)
{
    /* This handler only gets invoked if AO=1 or higher on the local
       device.  Otherwise, xbee_io_response_handler() is invoked. */

    _sxa_io_process_response(&envelope->ieee_address, envelope->payload, envelope->length);
...
/*** EndHeader */
_xbee_sxa_debug int _sxa_remote_is_cmd_response_handler(xbee_dev_t* xbee, const void FAR* raw, uint16_t length,
                                                        void FAR* context)
{
    ...
    if (XBEE_AT_RESP_STATUS(resp->header.status) == XBEE_AT_RESP_SUCCESS) {
        if (resp->header.command.w == is.w) {
            /* I/O sample record */
            _sxa_io_process_response(&resp->header.ieee_address, resp->value,
...
/*** EndHeader */
_xbee_sxa_debug int _sxa_local_is_cmd_response_handler(xbee_dev_t* xbee, const void FAR* raw, uint16_t length,
                                                       void FAR* context)
{
    static const xbee_at_cmd_t is              = {{'I', 'S'}};
    const xbee_frame_local_at_resp_t FAR* resp = raw;
    ...
    if (XBEE_AT_RESP_STATUS(resp->header.status) == XBEE_AT_RESP_SUCCESS) {
        if (resp->header.command.w == is.w) {
            ...
            _sxa_io_process_response(&xbee->wpan_dev.address.ieee, resp->value,
...
/*** EndHeader */
_xbee_sxa_debug int sxa_get_digital_input(const sxa_node_t FAR* sxa, uint_fast8_t index)
{
...
/*** EndHeader */
_xbee_sxa_debug int sxa_get_digital_output(const sxa_node_t FAR* sxa, uint_fast8_t index)
{
...
/*** EndHeader */
_xbee_sxa_debug int16_t sxa_get_analog_input(const sxa_node_t FAR* sxa, uint_fast8_t index)
{
...
/*** EndHeader */
_xbee_sxa_debug int sxa_set_digital_output(sxa_node_t FAR* sxa, uint_fast8_t index,
                                           enum xbee_io_digital_output_state state)
{
    return xbee_io_set_digital_output(sxa->xbee, &sxa->io, index, state, sxa->addr_ptr);
}

/*** BeginHeader sxa_io_configure */
/*** EndHeader */
_xbee_sxa_debug int sxa_io_configure(sxa_node_t FAR* sxa, uint_fast8_t index, enum xbee_io_type type)
{
    return xbee_io_configure(sxa->xbee, &sxa->io, index, type, sxa->addr_ptr);
}

/*** BeginHeader sxa_io_set_options */
/*** EndHeader */
_xbee_sxa_debug int sxa_io_set_options(sxa_node_t FAR* sxa, uint16_t sample_rate, uint16_t change_mask)
{
    return xbee_io_set_options(sxa->xbee, &sxa->io, sample_rate, change_mask, sxa->addr_ptr);
}

/*** BeginHeader sxa_io_query */
/*** EndHeader */
_xbee_sxa_debug int sxa_io_query(sxa_node_t FAR* sxa) { return xbee_io_query(sxa->xbee, &sxa->io, sxa->addr_ptr); }

/*** BeginHeader sxa_io_query_status */
/*** EndHeader */
_xbee_sxa_debug int sxa_io_query_status(sxa_node_t FAR* sxa) { return xbee_io_query_status(&sxa->io); }

/*** BeginHeader sxa_io_dump */
/*** EndHeader */
_xbee_sxa_debug void sxa_io_dump(sxa_node_t FAR* sxa)
{
    // Debug only
...
const FAR xbee_atcmd_reg_t _sxa_init_query_regs[] = {
    ...
    XBEE_ATCMD_REG_SET_8('N', 'O', 2), XBEE_ATCMD_REG_END_CMD('N', 'D')};

_xbee_sxa_debug sxa_node_t FAR* sxa_init_or_exit(xbee_dev_t* xbee, const wpan_endpoint_table_entry_t* ep_table,
                                                 int verbose)
{
    ...
    xbee_cmd_list_execute(xbee,
                          //&clc,
                          &sxa->io.clc, _sxa_init_query_regs, &sxa->id, NULL);
...
void _sxa_cmd_list_cache_cb(const xbee_cmd_response_t FAR* response, const struct xbee_atcmd_reg_t FAR* reg,
                            void FAR* base)
{
    sxa_cached_t FAR* c;
...
void _sxa_launch_update(sxa_node_t FAR* sxa, const struct _xbee_reg_descr_t FAR* xrd, uint16_t cache_group)
{
    ...
    if (list) {
        xbee_cmd_list_execute(sxa->xbee, &sxa->io.clc, list, sxa, sxa->addr_ptr);
...
void sxa_tick(void)
{
    ...
    for (sxa = sxa_table; sxa; sxa = sxa->next) {
        if (!sxa->queued && !sxa->doing_group) {
            ...
            if (sxa_cache_status(sxa, NULL, SXA_CACHED_DEVICE_INFO) == _SXA_CACHED_UNKNOWN)
                _sxa_launch_update(sxa, NULL, SXA_CACHED_DEVICE_INFO);
            else if (sxa_cache_status(sxa, NULL, SXA_CACHED_NODE_ID) == _SXA_CACHED_UNKNOWN)
                _sxa_launch_update(sxa, NULL, SXA_CACHED_NODE_ID);
            else if (sxa_cache_status(sxa, NULL, SXA_CACHED_IO_CONFIG) == _SXA_CACHED_UNKNOWN)
        ...
        default:
            if (xrd) {
                ...
                if (!err) {
                    ...
                    err = sxa_cache_status(sxa, xrd, xrd->sxa_cache_group) == _SXA_CACHED_ERROR;
                }
#ifdef SXA_CACHE_VERBOSE
                printf("%s: node %u got result from %s (err=%d)\n", __FUNCTION__, sxa->index, xrd->alias, err);
#endif
                _sxa_set_cache_status(sxa, xrd, xrd->sxa_cache_group, err ? _SXA_CACHED_ERROR : _SXA_CACHED_OK);
                ++sxa->q_index;
            } else {
                ...
                if (sxa->doing_group) {
                    ...
                    printf("%s: node %u got result from group %d (err=%d)\n", __FUNCTION__, sxa->index,
                           sxa->doing_group->id, err);
#endif
                    _sxa_set_cache_status(sxa, NULL, sxa->doing_group->id, err ? _SXA_CACHED_ERROR : _SXA_CACHED_OK);
                }
            }
            ...
            while (sxa->q_index < sxa->nqueued) {
                xrd = sxa->queued[sxa->q_index];
                if (sxa_cache_status(sxa, xrd, xrd->sxa_cache_group) == _SXA_CACHED_PENDING) {
...
/*** EndHeader */
void _sxa_set_cache_status(sxa_node_t FAR* sxa, const _xbee_reg_descr_t FAR* zb, uint16_t cache_group,
...
int sxa_find_pending_cache(sxa_node_t FAR* sxa, const _xbee_reg_descr_t FAR* zb)
{
    ...
    for (i = 0; i < sxa->nqueued; ++i) {
        if (sxa->queued[i] != zb)
            continue;
        if (sxa_cache_status(sxa, zb, zb->sxa_cache_group) & (_SXA_CACHED_BUSY | _SXA_CACHED_PENDING))
            return 1;
    }
    return 0;
}

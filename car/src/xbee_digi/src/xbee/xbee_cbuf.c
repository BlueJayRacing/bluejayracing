...
/*** EndHeader */
unsigned int xbee_cbuf_used(xbee_cbuf_t FAR* cbuf) { return (cbuf->tail - cbuf->head) & cbuf->mask; }

/*** BeginHeader xbee_cbuf_free */
/*** EndHeader */
unsigned int xbee_cbuf_free(xbee_cbuf_t FAR* cbuf) { return (cbuf->head - cbuf->tail - 1) & cbuf->mask; }

/*** BeginHeader xbee_cbuf_flush */
/*** EndHeader */
void xbee_cbuf_flush(xbee_cbuf_t FAR* cbuf) { cbuf->head = cbuf->tail; }

...
{
    static const char* _xbee_disc_device_type[] = {"Coord", "Router", "EndDev", "???"};
...
void xbee_disc_node_id_dump(const xbee_node_id_t FAR* ni)
{
    if (ni != NULL) {
        printf("Address:%08" PRIx32 "-%08" PRIx32 " 0x%04x  "
               ...
               be32toh(ni->ieee_addr_be.l[0]), be32toh(ni->ieee_addr_be.l[1]), ni->network_addr, ni->parent_addr,
...
int xbee_disc_nodeid_cluster_handler(const wpan_envelope_t FAR* envelope, void FAR* context)
{
    ...
    return _xbee_disc_parse_and_pass((xbee_dev_t*)envelope->dev, envelope->payload, envelope->length);
}
...
int xbee_disc_atnd_response_handler(xbee_dev_t* xbee, const void FAR* raw, uint16_t length, void FAR* context)
{
    ...
    if (resp->header.command.w == nd.w) {
        if (XBEE_AT_RESP_STATUS(resp->header.status) == XBEE_AT_RESP_SUCCESS) {
            ...
            return _xbee_disc_parse_and_pass(xbee, resp->value, length - offsetof(xbee_frame_local_at_resp_t, value));
...
int xbee_disc_nodeid_frame_handler(xbee_dev_t* xbee, const void FAR* raw, uint16_t length, void FAR* context)
{
    ...
    return _xbee_disc_parse_and_pass(xbee, &frame->node_data, length - offsetof(xbee_frame_node_id_t, node_data));
}

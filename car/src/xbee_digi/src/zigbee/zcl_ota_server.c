...
{
    resp->frame_control = (ZCL_FRAME_DISABLE_DEF_RESP | ZCL_FRAME_DIRECTION) ^
...
int zcl_ota_next_image_resp(zcl_command_t* zcl_req)
{
    ...
    if ((request->field_control == ZCL_OTA_QUERY_NEXT_IMAGE_FIELD_HARDWARE_VER && zcl_req->length == sizeof *request) ||
        (request->field_control == 0 && zcl_req->length == sizeof *request - 2)) {
        ota = zcl_ota_get_upgrade_source(&zcl_req->envelope->ieee_address, &request->id, zcl_req->command);
        frame.response.status = ota == NULL ? ZCL_STATUS_NO_IMAGE_AVAILABLE : ZCL_STATUS_SUCCESS;
    } else {
...
int zcl_ota_image_block_resp(zcl_command_t* zcl_req)
{
    ...
    if (request->field_control == 0 && zcl_req->length == sizeof *request - 10) {
        ota = zcl_ota_get_upgrade_source(&zcl_req->envelope->ieee_address, &request->id, zcl_req->command);
        if (ota == NULL) {
    ...
    int bytes_read = ota->read_handler(ota, &frame.data[0], file_offset, data_size, &zcl_req->envelope->ieee_address);
    ...
    return zcl_send_response(zcl_req, &frame, sizeof frame - ZCL_OTA_SERVER_MAX_BLOCK_SIZE + data_size);
}
...
int zcl_ota_upgrade_end_resp(zcl_command_t* zcl_req)
{
    ...
    frame.response.upgrade_time_le = htole32(zcl_ota_get_upgrade_time(&zcl_req->envelope->ieee_address, ota));
...
int zcl_ota_upgrade_cluster_handler(const wpan_envelope_t FAR* envelope, void FAR* context)
{
    ...
    if (ZCL_CMD_MATCH(&zcl.frame_control, GENERAL, CLIENT_TO_SERVER, CLUSTER)) {
#ifdef ZCL_OTA_VERBOSE

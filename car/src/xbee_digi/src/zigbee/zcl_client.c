...
*/
zcl_client_debug int zcl_process_read_attr_response(zcl_command_t* zcl, const zcl_attribute_base_t FAR* attr_table)
{
    ...
    while (write_rec.status == ZCL_STATUS_SUCCESS && write_rec.buffer < payload_end) {
        write_rec.flags  = ZCL_ATTR_WRITE_FLAG_ASSIGN | ZCL_ATTR_WRITE_FLAG_READ_RESP;
...
zcl_client_debug int zcl_client_read_attributes(wpan_envelope_t FAR* envelope, const zcl_client_read_t* client_read)
{
    ...
    if (client_read->mfg_id == ZCL_MFG_NONE) {
        zcl_req.header.u.std.frame_control = ZCL_FRAME_TYPE_PROFILE | ZCL_FRAME_GENERAL | ZCL_FRAME_CLIENT_TO_SERVER;
        request_start                      = &zcl_req.header.u.std.frame_control;
    } else {
        zcl_req.header.u.mfg.mfg_code_le = htole16(client_read->mfg_id);
        zcl_req.header.u.mfg.frame_control =
            ZCL_FRAME_TYPE_PROFILE | ZCL_FRAME_MFG_SPECIFIC | ZCL_FRAME_CLIENT_TO_SERVER;
                                                   ...
                                                   const wpan_envelope_t FAR* envelope)
{
    ...
    if (zdo_rsp->header.status != ZDO_STATUS_SUCCESS || zdo_rsp->header.match_length == 0) {
#ifdef ZCL_CLIENT_VERBOSE
    ...
    if (err != 0) {
#ifdef ZCL_CLIENT_VERBOSE
        printf("%s: %s returned %d\n", __FUNCTION__, "wpan_envelope_reply", err);
#endif
        return WPAN_CONVERSATION_CONTINUE;
    }

    // assert that network_address == zdo_rsp->header.network_addr_le?
    if (reply_envelope.network_address != le16toh(zdo_rsp->header.network_addr_le)) {
#ifdef ZCL_CLIENT_VERBOSE
    ...
    // the encryption flag)
    cluster = wpan_cluster_match(client_read.cluster_id, WPAN_CLUST_FLAG_CLIENT, client_read.ep->cluster_table);
    if (cluster) {
...
/**
   ...
   @retval  !0 error sending request
*/
zcl_client_debug int zcl_find_and_read_attributes(wpan_dev_t* dev, const uint16_t* clusters,
                                                  const zcl_client_read_t FAR* cr)
{
    return zdo_send_match_desc(dev, clusters, cr->ep->profile_id, _zcl_send_read_from_zdo_match, cr);
}
...
*/
zcl_client_debug int zdo_send_match_desc(wpan_dev_t* dev, const uint16_t* clusters, uint16_t profile_id,
                                         wpan_response_fn callback, const void FAR* context)
{
    ...
    frame_length =
        zdo_match_desc_request(buffer, sizeof(buffer), WPAN_NET_ADDR_BCAST_NOT_ASLEEP, profile_id, clusters, NULL);
    if (frame_length < 0) {
#ifdef ZCL_CLIENT_VERBOSE
    ...
    trans = wpan_conversation_register(zdo_endpoint_state(dev), callback, context, 60);
    if (trans < 0) {
#ifdef ZCL_CLIENT_VERBOSE
...
#endif
zcl_client_debug int zcl_create_attribute_records(void FAR* buffer, uint8_t bufsize,
                                                  const zcl_attribute_base_t FAR** p_attr_list)
{
    ...
    if (p_attr_list == NULL || buffer == NULL || (attr = *p_attr_list) == NULL) {
...
const char* _zcl_format_utctime(char dest[40], zcl_utctime_t utctime)
{
    ...
    if (dest != NULL) {
        if (utctime == ZCL_UTCTIME_INVALID) {
            ...
            sprintf(dest, "%04u-%02u-%02u %02u:%02u:%02u UTC", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour,
                    tm.tm_min, tm.tm_sec);
        }
    }
...
int zcl_print_array_value(const void* value, int value_length)
{
    ...
    for (i = 0, offset = 3; i < count && offset < value_length; ++i) {
        printf("\t[%2u] = ", i);
        result = zcl_print_attribute_value(type, &b[offset], value_length - offset);
        if (result < 0) {
...
int zcl_print_attribute_value(uint8_t type, const void* value, int value_length)
{
    ...
    if (!hexdump)
        ...
        case ZCL_TYPE_ENUM_8BIT:
            ...
            } else if ((type >= ZCL_TYPE_BITMAP_8BIT && type <= ZCL_TYPE_BITMAP_32BIT) || type == ZCL_TYPE_ID_CLUSTER ||
                       type == ZCL_TYPE_ID_ATTRIB) {

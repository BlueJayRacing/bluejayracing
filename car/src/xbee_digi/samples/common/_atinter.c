...
int process_command_remote(xbee_dev_t* xbee, const char* cmdstr, const addr64 FAR* ieee)
{
    char cmdptr[2];
    const char* param;
    int16_t request;
...
int process_command(xbee_dev_t* xbee, const char* cmdstr) { return process_command_remote(xbee, cmdstr, NULL); }

void handle_at_cmd(xbee_dev_t* xbee, char* command) { process_command(xbee, command); }
...
const command_t command_list[] = {
    {"HV", OUTPUT_HEX | FLAG_ANYDEVICE, "Hardware Version", "Read modem hardware verison number."},

    {"VR", OUTPUT_HEX | FLAG_ANYDEVICE, "Firmware Version", "Read modem firmware version number."},

    {"VL", OUTPUT_STRING | FLAG_DIGIMESH900, "Version (Long)",
     "Read detailed version information, including build date/time."},

    // Networking Registers, ZNet/ZB

    {"ID", OUTPUT_HEX | FLAG_ZNET | FLAG_ROUTER, "PAN ID",
    ...
    {"ID", OUTPUT_HEX | FLAG_ZB | FLAG_SE | FLAG_ROUTER, "PAN ID",
     "Set or read the PAN ID.  If you set the ID, you must write it\n"
     "\tto non-volatile memory (\"WR\") and then reset the network\n"
     "\tsoftware (\"NR\").  (any 64-bit value, 0 for random)"},

    {"SC", OUTPUT_HEX | FLAG_ZNET | FLAG_ZB | FLAG_SE | FLAG_ANYNODETYPE, "Scan Channels",
     "Set or read the list of channels to scan when joining or\n"
     "\tsetting up a network."},

    {"CH", OUTPUT_HEX | FLAG_ZNET | FLAG_ZB | FLAG_SE | FLAG_ANYNODETYPE, "Operating Channel",
     "Read the current channel.  Will be zero if we are not\n"
     "\tassociated with a network."},

    {"OP", OUTPUT_HEX | FLAG_ZNET | FLAG_ZB | FLAG_SE | FLAG_ANYNODETYPE, "Operating PAN ID",
     "Read the operating PAN ID."},

    {"OI", OUTPUT_HEX | FLAG_ZB | FLAG_SE | FLAG_ANYNODETYPE, "Operating 16-bit PAN ID",
     "Read the operating 16-bit PAN ID (ZigBee networks)."},

    // Networking Registers, DigiMesh

    {"ID", OUTPUT_HEX | FLAG_DIGIMESH900, "Network ID",
    ...
    {"HP", OUTPUT_DECIMAL | FLAG_DIGIMESH900, "Channel Hopping Pattern",
    ...
    {"BH", OUTPUT_DECIMAL | FLAG_ANYDEVICE, "Broadcast Radius", "Set or read the maximum number of broadcast hops."},

    // Addressing

    {"SH", OUTPUT_HEX | FLAG_ANYDEVICE, "Serial Number (High)", "Read the upper half of the 64-bit IEEE address."},

    {"SL", OUTPUT_HEX | FLAG_ANYDEVICE, "Serial Number (Low)", "Read the lower half of the 64-bit IEEE address."},

    {"MY", OUTPUT_HEX | FLAG_ZNET | FLAG_ZB | FLAG_SE | FLAG_ANYNODETYPE, "Network Address",
     "Read the 16-bit network address.  Will be 0xFFFE if not\n"
     "\tassociated with a network."},

    {"MP", OUTPUT_HEX | FLAG_ZNET | FLAG_ZB | FLAG_SE | FLAG_ENDDEV, "Parent Address",
     "Read the 16-bit network address of our parent node."},

    {"NI", OUTPUT_STRING | (FLAG_ANYDEVICE ^ FLAG_SE), "Node Identifier",
     "Set or read the node identifier (up to 20 characters)."},

    {"NT", OUTPUT_DECIMAL | FLAG_ANYDEVICE, "Node Discovery Timeout",
     "Set or read the node discovery timeout (0-255 x 100ms)."},

    {"SD", OUTPUT_DECIMAL | FLAG_ZNET | FLAG_ZB | FLAG_SE | FLAG_ANYNODETYPE, "Scan Duration",
     "Set or read the channel scan duration (0-7)."},

    {"NJ", OUTPUT_DECIMAL | FLAG_ZNET | FLAG_ZB | FLAG_SE | FLAG_ANYNODETYPE, "Node Join Time",
     "Set or read the node join time (0-255 seconds)."},

    {"JV", OUTPUT_DECIMAL | FLAG_ZNET | FLAG_ZB | FLAG_ROUTER, "Channel Verification",
    ...
    {"SM", OUTPUT_DECIMAL | FLAG_DIGIMESH900, "Sleep Mode", "Set or read the Sleep Mode setting (0, 7 or 8)."},

    {"SO", OUTPUT_HEX | FLAG_DIGIMESH900, "Sleep Options", "Set or read the Sleep Options setting (bitfield)."},

    {"ST", OUTPUT_DECIMAL | FLAG_DIGIMESH900, "Wake Time", "Set or read the Wake Time setting (milliseconds)."},

    {"OW", OUTPUT_DECIMAL | FLAG_DIGIMESH900, "Operational Wake Time",
    ...
    {"SP", OUTPUT_DECIMAL | FLAG_DIGIMESH900, "Sleep Period", "Set or read the Sleep Period setting (units of 10ms)."},

    {"OS", OUTPUT_DECIMAL | FLAG_DIGIMESH900, "Operational Sleep Period",
    ...
    {"SS", OUTPUT_HEX | FLAG_DIGIMESH900, "Sleep Status", "Read the Sleep Status bitfield."},

    // Diagnostic Commands

    {"AI", OUTPUT_HEX | FLAG_ANYDEVICE, "Assocation Indicator",
     "Read the association progress indicator (0 = associated)."},

    {"NP", OUTPUT_DECIMAL | FLAG_ANYDEVICE, "Network Payload",
     "Read the maximum number of RF payload bytes that can be sent\n"
     "\tin a single transmission."},

    {"%V", OUTPUT_DECIMAL | FLAG_ANYDEVICE, "Supply Voltage", "Read voltage on Vcc pin in mV."},

    {"TP", OUTPUT_DECIMAL | FLAG_DIGIMESH900, "Temperature", "Read module temperature (in Celsius)."},

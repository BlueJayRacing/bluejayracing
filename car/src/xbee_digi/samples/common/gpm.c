...
int upload_next_page()
{
    ...
    if (!complete) {
        ...
        maxcount  = upload_pagesize == 0 ? xbee_gpm_max_write(&my_xbee.wpan_dev) : upload_pagesize;
    ...
    result = xbee_gpm_write(&envelope_self, upload_offset / blocksize, upload_offset % blocksize, bytesread, buffer);
...
int gpm_response(const wpan_envelope_t FAR* envelope, void FAR* context)
{
    ...
    case XBEE_GPM_CMD_PLATFORM_INFO_RESP: {
        ...
        printf("Platform Info: status 0x%02X, %u blocks, %u bytes/block = %" PRIu32 " bytes\n",
               frame->header.response.status, blocks, blocksize, (uint32_t)blocks * blocksize);
    } break;
    ...
    case XBEE_GPM_CMD_ERASE_THEN_WRITE_RESP:
        printf("Write to offset %u of block %u response: status 0x%02X\n",
               be16toh(frame->header.response.start_index_be), be16toh(frame->header.response.block_num_be),
               frame->header.response.status);
...
const wpan_cluster_table_entry_t digi_device_clusters[] = {
    {DIGI_CLUST_MEMORY_ACCESS, gpm_response, NULL, WPAN_CLUST_FLAG_INOUT | WPAN_CLUST_FLAG_NOT_ZCL},
        ...
        if (linelen == ENODATA || !strcmpi(cmdstr, "quit")) {
            ...
            if (parse_uint16(params, &cmdstr[5], 3) == 3) {
                printf("Read %u bytes from offset %u of block %u (result %d)\n", params[2], params[1], params[0],
                       xbee_gpm_read(&envelope_self, params[0], params[1], params[2]));
            } else {
...
const xbee_dispatch_table_entry_t xbee_frame_handlers[] = {XBEE_FRAME_HANDLE_LOCAL_AT, XBEE_FRAME_MODEM_STATUS_DEBUG,
                                                           ...
                                                           XBEE_FRAME_HANDLE_RX_EXPLICIT, XBEE_FRAME_TABLE_END};

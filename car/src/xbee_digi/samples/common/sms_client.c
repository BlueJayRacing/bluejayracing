...
#define XBEE_SMS_RECEIVE_HANDLE {XBEE_FRAME_RECEIVE_SMS, 0, receive_handler, NULL}

const xbee_dispatch_table_entry_t xbee_frame_handlers[] = {XBEE_FRAME_HANDLE_LOCAL_AT, XBEE_SMS_RECEIVE_HANDLE,
...
int receive_handler(xbee_dev_t* xbee, const void FAR* raw, uint16_t length, void FAR* context)
{
    ...
    printf("Message from %.*s:\n%.*s\n", XBEE_SMS_MAX_PHONE_LENGTH, sms->phone,
           (int)(length - offsetof(xbee_frame_receive_sms_t, message)), sms->message);

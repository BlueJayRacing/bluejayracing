    ...
    do {
        ...
        if (entry.name_len > 0) {
            if (entry.is_dir) {
                ...
                printf("%*" PRIu32 " %s%c\n", 7, entry.filesize, entry.name, entry.is_secure ? '#' : ' ');
            }
        }
...
int send_file_write(xbee_dev_t* xbee)
{
    ...
    if ((xbee->hardware_series & XBEE_HW_SERIES_MASK) == XBEE_HW_SERIES_CELLULAR) {
        read_size = 1491;
    } else {
    ...
    if (read_size == 0) {
        printf("ERROR: unrecognized ATHS/HV/VR combination (0x%X/0x%X/0x%" PRIX32 ")\n", xbee->hardware_series,
    ...
    return xbee_fs_req_send_data(xbee, &req, sizeof(req), buffer, bytes_read, TARGET_IEEE);
}

int process_file_open(xbee_dev_t* xbee, const void FAR* payload, uint16_t length)
{
...
int process_file_write(xbee_dev_t* xbee, const void FAR* payload, uint16_t length)
{
    const xbee_payload_fs_file_rw_resp_t FAR* response = payload;
    ...
    if (xfer_state.file_handle != be16toh(response->file_handle_be)) {
        ...
        if (xfer_state.current_offset != be32toh(response->offset_be)) {
            printf("Warning: unexpected FILE_WRITE offset (%" PRIu32 " != %" PRIu32 ")\n", be32toh(response->offset_be),
                   xfer_state.current_offset);
        } else {
...
int xbee_frame_file_system_resp(xbee_dev_t* xbee, const void FAR* payload, uint16_t length, void FAR* context)
{
    ...
    case XBEE_FS_CMD_ID_GET_PATH_ID:
        ...
        printf("Path ID 0x%04X: %.*s\n", path_id, length - 2, (uint8_t*)payload + 2);
        break;
...
static const xbee_node_id_t broadcast_target = {{{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFF, 0xFF}}};
...
// broadcast
void handle_broadcast_cmd(xbee_dev_t* xbee, char* command) { set_target(&broadcast_target); }
...
// rm <name>
void handle_rm_cmd(xbee_dev_t* xbee, char* command) { handle_oneparam_cmd(xbee, XBEE_FS_CMD_ID_DELETE, &command[2]); }
...
void handle_volume_cmd(xbee_dev_t* xbee, char* command)
{
    ...
    int result = xbee_fs_req_send_str(xbee, &fs_cmd_id, sizeof(fs_cmd_id), vol, TARGET_IEEE);

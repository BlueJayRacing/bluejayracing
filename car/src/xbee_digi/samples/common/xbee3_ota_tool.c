                                                           ...
                                                           uint8_t zcl_command)
{
    ...
    bool_t match = (id->mfg_code_le == ota_update.id.mfg_code_le && id->image_type_le == ota_update.id.image_type_le);
...
void handle_notify_cmd(xbee_dev_t* xbee, char* command)
{
    ...
    int result = zcl_ota_upgrade_image_notify(&xbee->wpan_dev, &ota_update.id, &sample_endpoints[1],
                                              WPAN_ENDPOINT_DIGI_DATA, ieee_addr_be, network_addr);
...
int ota_read(const zcl_ota_upgrade_source_t* source, void* dest, uint32_t offset, size_t bytes,
             const addr64* client_ieee_be)
{
    char buffer[ADDR64_STRING_LENGTH];
...
int load_ota_file(const char* filename)
{
    ...
    ota_context.data_offset = le16toh(ota_header.header_len_le) + sizeof(zcl_ota_element_t);
    ...
    printf("  Mfg 0x%04X  Type 0x%04X  Ver 0x%08" PRIX32 "  Length %u\n", le16toh(ota_update.id.mfg_code_le),
           le16toh(ota_update.id.image_type_le), le32toh(ota_update.id.file_version_le), ota_update.image_size);
...
int main(int argc, char* argv[])
{
    ...
    if (name_len < 4 ||
        (strcmpi(&filename[name_len - 4], ".ota") != 0 && strcmpi(&filename[name_len - 4], ".otb") != 0)) {

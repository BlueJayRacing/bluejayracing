...
wpan_cluster_table_entry_t zcl_cluster_table[] = {{0, &zcl_general_command, zcl_attributes_none, WPAN_CLUST_FLAG_INOUT},
...
const xbee_dispatch_table_entry_t xbee_frame_handlers[] = {XBEE_FRAME_HANDLE_LOCAL_AT, XBEE_FRAME_HANDLE_RX_EXPLICIT,
                                                           // XBEE_FRAME_MODEM_STATUS_DEBUG,
                                                           XBEE_FRAME_TABLE_END};

struct _endpoints sample_endpoints = {ZDO_ENDPOINT(zdo_ep_state),
...
char* format_time(char* dest, zcl_utctime_t utctime)
{
    ...
    if (utctime == ZCL_UTCTIME_INVALID) {
        ...
        if (walker_use_gmtime) {
            ...
            sprintf(dest, "%04u-%02u-%02u %02u:%02u:%02uZ", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour,
                    tm.tm_min, tm.tm_sec);
        } else {
            t  = utctime + ZCL_TIME_EPOCH_DELTA;
            tm = *localtime(&t);
            sprintf(dest, "%04u-%02u-%02u %02u:%02u:%02u", tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday, tm.tm_hour,
                    tm.tm_min, tm.tm_sec);
        }
    }
...
int walker_active_ep_resp(wpan_conversation_t FAR* conversation, const wpan_envelope_t FAR* envelope)
{
    const XBEE_PACKED(, {
        uint8_t transaction;
...
int walker_simple_desc_resp(wpan_conversation_t FAR* conversation, const wpan_envelope_t FAR* envelope)
{
    const XBEE_PACKED(, {
        uint8_t transaction;
    ...
    if (envelope == NULL) {
        ...
        printf("Endpoint 0x%02X  Profile 0x%04X  Device 0x%04X  Ver 0x%02X\n", walker.current_endpoint,
               walker.profile_id, le16toh(response->descriptor.device_id_le), response->descriptor.device_version);
        ...
        overparse = (uint8_t FAR*)cluster_le - ((uint8_t FAR*)envelope->payload + envelope->length);
        if (overparse) {
...
#endif
            wpan_envelope_create(&envelope, walker.dev, &walker.target.ieee, walker.target.network);
            if (zdo_simple_desc_request(&envelope, walker.target.network, walker.current_endpoint,
                                        walker_simple_desc_resp, NULL) == 0) {
                return WALKER_WAIT;
            }
...
int walker_process_discover_attr_resp(wpan_conversation_t FAR* conversation, const wpan_envelope_t FAR* envelope)
{
    zcl_command_t zcl;
...
void walker_zcl_init(wpan_envelope_t* envelope, zcl_header_nomfg_t* zcl_header, uint16_t zcl_length)
{
    wpan_envelope_create(envelope, walker.dev, &walker.target.ieee, walker.target.network);
    ...
    zcl_header->frame_control =
        ZCL_FRAME_CLIENT_TO_SERVER | ZCL_FRAME_TYPE_PROFILE | ZCL_FRAME_GENERAL | ZCL_FRAME_DISABLE_DEF_RESP;
...
enum walker_status_t walker_send_discover_attr_req(void)
{
    ...
    trans = wpan_conversation_register(&zcl_ep_state, walker_process_discover_attr_resp, NULL, WALKER_ZCL_TIMEOUT);
    ...
    if (walker.attribute.count == 0) {
        ...
        zcl.req.start_attrib_id_le = htole16(1 + le16toh(walker.attribute.list[walker.attribute.count - 1].id_le));
    }
...
{
    printf("      attr 0x%04X, type 0x%02X (%s) ", le16toh(attr->id_le), attr->type, zcl_type_name(attr->type));
}

int walker_process_read_attr_resp(wpan_conversation_t FAR* conversation, const wpan_envelope_t FAR* envelope)
{
    ...
    if (envelope == NULL) {
        ...
        if (resp->id_le != attr->id_le) {
            ...
            if (resp->type != attr->type) {
                printf("WARNING: Read Attr Resp had different type (0x%02x)", resp->type);
            }
        }
    }
...
int walker_send_read_attr(zcl_rec_attrib_report_t* attr)
{
    ...
    trans = wpan_conversation_register(&zcl_ep_state, walker_process_read_attr_resp, attr, WALKER_ZCL_TIMEOUT);
...
enum walker_status_t walker_tick(void)
{
    ...
    case WALKER_NWK_ADDR_REQ:
        ...
        if (zdo_send_nwk_addr_req(walker.dev, &walker.target.ieee, &walker.target.network) == 0) {
    ...
    case WALKER_ACTIVE_EP_REQ:
        ...
        wpan_envelope_create(&envelope, walker.dev, &walker.target.ieee, walker.target.network);
        if (zdo_send_descriptor_req(&envelope, ZDO_ACTIVE_EP_REQ, walker.target.network, walker_active_ep_resp, NULL) ==
            0) {

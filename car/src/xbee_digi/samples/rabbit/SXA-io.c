...
const wpan_cluster_table_entry_t digi_data_clusters[] = {
    ...
    XBEE_IO_CLIENT_CLUST_ENTRY, XBEE_DISC_CLIENT_CLUST_ENTRY, WPAN_CLUST_ENTRY_LIST_END};
...
int main(void)
{
    ...
    while (1) {
        ...
        if (!strncmpi(cmdstr, "menu", 4)) {
            ...
            ionum  = cmdstr[1] - '0';
            status = sxa_set_digital_output(sxa, ionum,
            ...
            ionum = cmdstr[1] - '0';
            status =
                sxa_io_configure(sxa, ionum, cmdstr[0] == '.' ? XBEE_IO_TYPE_DIGITAL_INPUT : XBEE_IO_TYPE_ANALOG_INPUT);
            printf("I/O command status %d\n", status);
        } else if (cmdstr[0] == '#' || cmdstr[0] == '$') {
...
const xbee_dispatch_table_entry_t xbee_frame_handlers[] = {
    ...
    XBEE_FRAME_HANDLE_IS_RESPONSE, XBEE_FRAME_HANDLE_ND_RESPONSE, XBEE_FRAME_HANDLE_RX_EXPLICIT,

...
int main(void)
{
    ...
    while (1) {
        ...
        if (!strncmpi(cmdstr, "menu", 4)) {
            ...
            have_target ? process_command_remote(&my_xbee, cmdstr, &target_ieee) : process_command(&my_xbee, cmdstr);
        }
    }
}
...
int xbee_nd_cmd_response_handler(xbee_dev_t* xbee, const void FAR* raw, uint16_t length, void FAR* context)
{
    ...
    if (XBEE_AT_RESP_STATUS(resp->header.status) == XBEE_AT_RESP_SUCCESS) {
        if (resp->header.command.w == nd.w) {
            ...
            if (xbee_disc_nd_parse(&node_id, resp->value, length - offsetof(xbee_frame_local_at_resp_t, value)) == 0) {

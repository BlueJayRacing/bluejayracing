...
const wpan_cluster_table_entry_t digi_data_clusters[] = {
    ...
    XBEE_DISC_CLIENT_CLUST_ENTRY, WPAN_CLUST_ENTRY_LIST_END};
...
int passive_open(void)
{
    ...
    state = LISTEN;
    return tcp_extlisten(&sock, IF_LOOPBACK, SXA_STREAM_PORT, 0, SXA_STREAM_PORT, NULL, 0, 0, 0);
}
...
int active_open(int sxa_index)
{
    ...
    return tcp_extopen(&sock, IF_LOOPBACK, SXA_STREAM_PORT, SXA_INDEX_TO_IP(sxa_index), SXA_STREAM_PORT, NULL, 0, 0);
}
...
void state_machine(void)
{
    ...
    if (state == ESTABLISHED_PASSIVE || state == ESTABLISHED_ACTIVE) {
...
int main(void)
{
    ...
    while (1) {
        ...
        } else if (state == ESTABLISHED_PASSIVE || state == ESTABLISHED_ACTIVE) {
...
const xbee_dispatch_table_entry_t xbee_frame_handlers[] = {
    ...
    XBEE_FRAME_HANDLE_LOCAL_AT, XBEE_FRAME_HANDLE_REMOTE_AT,

    // Standard handlers (required) for SXA to work properly
    XBEE_FRAME_HANDLE_ND_RESPONSE, XBEE_FRAME_HANDLE_RX_EXPLICIT,

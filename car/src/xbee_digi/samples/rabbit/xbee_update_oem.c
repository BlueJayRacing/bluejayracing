...
int configure_modem(xbee_dev_t* xbee)
{
    ...
    for (i = 0; i < sizeof(settings) / sizeof(*settings); ++i) {
        ...
        bytesread = 0;
        do {
            retval = xbee_atmode_read_response(xbee, response, sizeof(response), &bytesread);
        } while (retval == -EAGAIN);
...
void main()
{
    ...
    xbee_fw_buffer_init(&fw, ximport_length(FIRMWARE), ximport_data(FIRMWARE));
    ...
    do {
        ...
        printf("\x1B[K"
               " %ls\r",
               xbee_fw_status_oem(&fw.source, buffer));
    } while (result == 0);

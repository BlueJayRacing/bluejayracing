...
#define XBEE_CMD_FLAG_USER_MASK (XBEE_CMD_FLAG_QUEUE_CHANGE | XBEE_CMD_FLAG_REUSE_HANDLE)
    ///@}
...
*/
typedef void (*xbee_command_list_fn)(const xbee_cmd_response_t FAR* response, const struct xbee_atcmd_reg_t FAR* reg,
                                     void FAR* base);
...
#define XBEE_ATCMD_REG(c1, c2, type, obj, field)                                                                       \
    {{{c1, c2}}, 0, type, NULL, (uint8_t)sizeof((*(obj*)NULL).field), (uint8_t)offsetof(obj, field)}
...
#define XBEE_ATCMD_REG_MOVE_THEN_CB(c1, c2, type, obj, field, cb, flags)                                               \
    {{{c1, c2}}, flags, type, cb, (uint8_t)sizeof((*(obj*)NULL).field), (uint8_t)offsetof(obj, field)}
...
int xbee_cmd_list_execute(xbee_dev_t* xbee, xbee_command_list_context_t FAR* clc, const xbee_atcmd_reg_t FAR* list,
...
extern FAR xbee_cmd_request_t xbee_cmd_request_table[XBEE_CMD_REQUEST_TABLESIZE];
